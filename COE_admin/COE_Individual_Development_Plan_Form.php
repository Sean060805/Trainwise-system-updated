<?php
/* ===================================================================
   COE IDP SUBMISSIONS — Department view for College of Engineering
   =================================================================== */
...
function processSignatureUpload($file, $role, $formId)
{
    ...
    $uploadDir = '../uploads/signatures/coe/';
...
/*
|--------------------------------------------------------------------------
| Main employee list - FILTERED TO COE DEPARTMENT
|--------------------------------------------------------------------------
*/
$query = "
    SELECT 
    ...
    WHERE f.status = 'submitted' AND u.department = 'COE'
...
$statsResult = $con->query("
    ...
    FROM users 
    WHERE department = 'COE' AND role = 'user' 
    AND teaching_status IS NOT NULL AND teaching_status != ''
");
if ($statsResult) {
    $coeStats['total_employees'] = $statsResult->fetch_assoc()['count'];
}
$coeStats['with_idp'] = count($employees);
...
$hrResult = $con->query("
    ...
    FROM idp_forms f
    JOIN users u ON f.user_id = u.id
    WHERE f.status = 'submitted_to_hr' AND u.department = 'COE'
");
if ($hrResult) {
    $coeStats['submitted_to_hr'] = $hrResult->fetch_assoc()['count'];
}
...
<head>
  ...
  <title>COE IDP Submissions · LSPU TNA</title>
...
<style>
  /* ==========================================================
     DESIGN SYSTEM — COE Green Accent Version
     ========================================================== */
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .btn-green { background:var(--green); color:#fff; box-shadow:0 8px 18px -10px rgba(6,95,70,0.6); }
  ...
  .view-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--green); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.85rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(6,95,70,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
  ...
  .department-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--green-soft); color:#064e3b; padding:0.32rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .emp-avatar { width:38px; height:38px; border-radius:10px; background:var(--green-soft); color:#064e3b; display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .search input:focus { outline:none; border-color:var(--green); box-shadow:0 0 0 4px rgba(6,95,70,0.12); }
  ...
  .search-go:hover { background:#047857; }
  ...
  .status-select:focus { outline:none; border-color:var(--green); box-shadow:0 0 0 4px rgba(6,95,70,0.12); }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="COE_Individual_Development_Plan_Form.php" class="rail-link active">
  ...
  <div class="app-main">
    <header class="topbar">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip" aria-label="Account">
          ...
          <span class="avatar-meta"><span class="nm"><?= htmlspecialchars($adminName) ?></span> <span class="rl">COE Admin</span></span>
        </div>
      </div>
    </header>
    ...
    <div class="content-scroll">
      <div class="content">
        <div class="page-head">
          <div>
            <p class="eyebrow">Individual Development Plans · COE</p>
            <h2>COE IDP Submissions</h2>
            <p>View and manage IDP forms submitted by College of Engineering faculty and staff.</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">Total COE Faculty</p>
                <p class="stat-value num"><?php echo $coeStats['total_employees']; ?></p>
          ...
          <div class="stat a-green">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $coeStats['with_idp']; ?></p>
                <p class="stat-sub"><?php echo $coeStats['total_employees'] > 0 ? round(($coeStats['with_idp'] / $coeStats['total_employees']) * 100, 1) : 0; ?>% of faculty</p>
              </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $coeStats['submitted_to_hr']; ?></p>
          ...
          <div class="card-body" style="padding:1.05rem 1.15rem;">
            <div class="filterbar">
              <div style="display:inline-flex;align-items:center;gap:0.55rem;height:42px;padding:0 0.9rem;border:1px solid var(--green);border-radius:var(--radius-sm);background:var(--green-soft);">
                <i class="ri-building-line" style="color:#064e3b;"></i>
                <span style="font-weight:600;color:#064e3b;font-size:0.85rem;">COE</span>
                <span style="font-size:0.7rem;color:#064e3b;opacity:0.7;">College of Engineering</span>
              </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p style="font-size:0.82rem; color:var(--muted);">
            ...
            of <span class="num"><?= count($employees) ?></span> COE employees with submitted IDPs
          </p>
        </div>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No IDP submissions found for COE</h4>
                        <p>COE faculty and staff will appear here once they submit their IDP forms.</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="signature-upload-card">
                    ...
                    <input
                      ...
                      class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-green file:text-white hover:file:bg-[#047857] cursor-pointer signature-file-input"
