<?php
/* ===================================================================
   COE — ASSESSMENT FORM SUBMISSIONS (College of Engineering only)
   -------------------------------------------------------------------
   This is a COE-scoped copy of "Assessment Form.php". EVERY query below
   is constrained to department = 'COE' — a COE admin can only ever see
   or act on COE records. UI matches COE.php (dark rail sidebar, green
   accent, dual LSPU + COE logos).

   Assumed to live in the same folder as COE.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as COE.php — keeps this page inside the protected COE area.
...
$forwardedDemandCount = getForwardedDemandCountForCollege($con, 'COE');
...
// ----- Delete handler (POST) — only allowed for COE-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a COE user.
        $coeDept = COE_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $coeDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — COE only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . COE_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — COE only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . COE_DEPT . "'";
...
// ----- Count total rows (respects active filters, COE only) ---------
...
// ----- Main paginated query (sorted by surname, COE only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all COE rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted COE faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always COE).
$coe_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . COE_DEPT . "' AND status = 'accepted'");
$coe_eligible = ($coe_eligible_result && $coe_eligible_result->num_rows > 0) ? (int)$coe_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $coe_eligible > 0 ? round(($total_rows / $coe_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>COE Assessment Forms · LSPU TNA</title>
  ...
  <link rel="stylesheet" href="../assets/css/tw-18.css">
<style>
  /* Design system — same tokens as COE.php / admin_page.php, green-accented rail. */
  :root {
    --bg:#f5f6f8; --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(6,95,70,0.05), transparent 60%);
    ...
    --accent:#065f46; --accent-700:#047857; --accent-soft:#d1fae5; --accent-ink:#064e3b;
    ...
    --rail:#0a2e1a; --rail-2:#1a4d2a; --rail-line:rgba(148,163,184,0.14);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .scope-badge { display:inline-flex; align-items:center; gap:0.4rem; background:var(--green-soft); color:#064e3b; padding:0.3rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; margin-left:0.5rem; vertical-align:middle; }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--green-soft); color:#064e3b; padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .search input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(6,95,70,0.12); }
  ...
  .filter-menu a.sel { background:var(--green-soft); color:#064e3b; font-weight:600; }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/coe-logo.png" alt="COE" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="COE.php" class="rail-link">
      ...
      <a href="COE_Assessment Form.php" class="rail-link active">
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>COE Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> COE only</span></h2>
            <p>Viewing submissions from College of Engineering faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
        ...
        <div class="grid-stats">
          <div class="stat a-green">
            <div class="stat-body">
              <div>
                <p class="stat-label">COE Records</p>
          ...
          <div class="stat a-grey">
            <div class="stat-body">
              ...
              <div class="stat-ico ico-grey"><i class="ri-user-line"></i></div>
            </div>
          </div>
          <div class="stat a-amber">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $coe_eligible ?> COE accepted users</p>
              </div>
        ...
        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to COE) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> COE records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Engineering</div>
                      </td>
                      <td>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No COE submissions found</h4>
                        <p>
...
<!-- HIDDEN EXPORT TABLE (COE rows only) -->
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Engineering</p></div>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF ENGINEERING";
  doc.setFontSize(13);
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[209,250,229], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });

  doc.save("COE_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
