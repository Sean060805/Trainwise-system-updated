...
/* ===================================================================
   ...
   or act on CHMT records. UI matches CHMT.php (dark rail sidebar, maroon
   accent, dual LSPU + CHMT logos).

   Assumed to live in the same folder as CHMT.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as CHMT.php — keeps this page inside the protected CHMT area.
...
// ----- Delete handler (POST) — only allowed for CHMT-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a CHMT user.
        $chmtDept = CHMT_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $chmtDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — CHMT only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . CHMT_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — CHMT only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . CHMT_DEPT . "'";
...
// ----- Count total rows (respects active filters, CHMT only) ---------
...
// ----- Main paginated query (sorted by surname, CHMT only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all CHMT rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted CHMT faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always CHMT).
$chmt_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . CHMT_DEPT . "' AND status = 'accepted'");
$chmt_eligible = ($chmt_eligible_result && $chmt_eligible_result->num_rows > 0) ? (int)$chmt_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $chmt_eligible > 0 ? round(($total_rows / $chmt_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>CHMT Assessment Forms · LSPU TNA</title>
...
<style>
  /* Design system — same tokens as CHMT.php / admin_page.php, maroon-accented rail. */
  :root {
    --bg:#f5f6f8; --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(159,18,57,0.05), transparent 60%);
    ...
    --maroon:#9f1239; --maroon-soft:#fdf2f8;
  }
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:32px; height:32px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--accent), #be123c); display:flex; align-items:center; justify-content:center; }
  ...
  .scope-badge { display:inline-flex; align-items:center; gap:0.4rem; background:var(--maroon-soft); color:#4c0519; padding:0.3rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; margin-left:0.5rem; vertical-align:middle; }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--maroon-soft); color:#4c0519; padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .search input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(159,18,57,0.12); }
  ...
  .filter-menu a.sel { background:var(--maroon-soft); color:#4c0519; font-weight:600; }
  ...
  .pg.active { background:var(--maroon); color:#fff; border-color:var(--maroon); }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/chmt-logo.png" alt="CHMT" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="CHMT_Assessment Form.php" class="rail-link active">
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>CHMT Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> CHMT only</span></h2>
            <p>Viewing submissions from College of Hospitality Management &amp; Tourism faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
          <button onclick="generatePDF()" class="btn btn-maroon">
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">CHMT Records</p>
          ...
          <div class="stat a-maroon">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $chmt_eligible ?> CHMT accepted users</p>
              </div>
              <div class="stat-ico ico-maroon"><i class="ri-percent-line"></i></div>
            </div>
          </div>
        </div>

        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to CHMT) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> CHMT records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Hospitality Management &amp; Tourism</div>
                      </td>
                      <td>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No CHMT submissions found</h4>
                        <p>
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Hospitality Management &amp; Tourism</p></div>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF HOSPITALITY MANAGEMENT AND TOURISM";
  doc.setFontSize(13);
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[253,242,248], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });

  doc.save("CHMT_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
