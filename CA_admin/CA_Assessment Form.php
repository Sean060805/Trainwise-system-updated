<?php
/* ===================================================================
   CA — ASSESSMENT FORM SUBMISSIONS (College of Agriculture only)
   -------------------------------------------------------------------
   This is a CA-scoped copy of "Assessment Form.php". EVERY query below
   is constrained to department = 'CA' — a CA admin can only ever see
   or act on CA records. UI matches CA.php (dark rail sidebar, green
   accent, dual LSPU + CA logos).

   Assumed to live in the same folder as CA.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as CA.php — keeps this page inside the protected CA area.
...
if (($_SESSION['user_role'] ?? '') !== 'admin_ca') {
    header("Location: ../index.php");
    exit();
}
...
$forwardedDemandCount = getForwardedDemandCountForCollege($con, 'CA');
...
// ----- Delete handler (POST) — only allowed for CA-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a CA user.
        $caDept = CA_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $caDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — CA only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . CA_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — CA only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . CA_DEPT . "'";
...
// ----- Count total rows (respects active filters, CA only) ---------
...
// ----- Main paginated query (sorted by surname, CA only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all CA rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted CA faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always CA).
$ca_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . CA_DEPT . "' AND status = 'accepted'");
$ca_eligible = ($ca_eligible_result && $ca_eligible_result->num_rows > 0) ? (int)$ca_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $ca_eligible > 0 ? round(($total_rows / $ca_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>CA Assessment Forms · LSPU TNA</title>
  ...
  <link rel="stylesheet" href="../assets/css/tw-25.css">
<style>
  /* Design system — same tokens as CA.php / admin_page.php, green-accented rail. */
  :root {
    --bg:#f5f6f8; --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(22,163,74,0.05), transparent 60%);
    ...
    --accent:#16a34a; --accent-700:#15803d; --accent-soft:#dcfce7; --accent-ink:#14532d;
    ...
    --rail:#0a2e0a; --rail-2:#1a4d1a; --rail-line:rgba(148,163,184,0.14);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-link.active { background:linear-gradient(100deg, rgba(22,163,74,0.26), rgba(22,163,74,0.08)); color:#fff; border-color:rgba(74,222,128,0.35); }
  ...
  .scope-badge { display:inline-flex; align-items:center; gap:0.4rem; background:var(--green-soft); color:#14532d; padding:0.3rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; margin-left:0.5rem; vertical-align:middle; }
  ...
  .stat.a-green  { --stat-accent:linear-gradient(90deg, #4ade80, #16a34a); }
  .stat.a-brown   { --stat-accent:linear-gradient(90deg, #ca8a04, #a16207); }
  ...
  .btn-primary { background:var(--accent); color:#fff; box-shadow:0 8px 18px -10px rgba(22,163,74,0.7); }
  ...
  .btn-green { background:var(--green); color:#fff; box-shadow:0 8px 18px -10px rgba(22,163,74,0.55); }
  .btn-green:hover { background:#15803d; }
  .btn-brown { background:var(--brown); color:#fff; box-shadow:0 8px 18px -10px rgba(161,98,7,0.55); }
  .btn-brown:hover { background:#854d0e; }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--green-soft); color:#14532d; padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .search input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(22,163,74,0.12); }
  ...
  .filter-menu a.sel { background:var(--green-soft); color:#14532d; font-weight:600; }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/ca.png" alt="CA" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="CA.php" class="rail-link">
        <i class="ri-dashboard-line"></i><span>Dashboard</span>
      </a>

      <a href="CA_Training_Demand.php" class="rail-link">
      ...
      <a href="CA_Assessment Form.php" class="rail-link active">
        <i class="ri-survey-line"></i><span>Assessment Form</span>
        <i class="ri-arrow-right-s-line chev"></i>
      </a>
      <a href="CA_Individual_Development_Plan_Form.php" class="rail-link">
        <i class="ri-contacts-book-2-line"></i><span>IDP Forms</span>
      </a>
      <a href="ca_eval.php" class="rail-link">
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>CA Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> CA only</span></h2>
            <p>Viewing submissions from College of Agriculture faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
        ...
        <div class="grid-stats">
          <div class="stat a-green">
            <div class="stat-body">
              <div>
                <p class="stat-label">CA Records</p>
          ...
          <div class="stat a-amber">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $ca_eligible ?> CA accepted users</p>
              </div>
        ...
        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to CA) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> CA records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Agriculture</div>
                      </td>
                      <td>
...
<!-- HIDDEN EXPORT TABLE (CA rows only) -->
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Agriculture</p></div>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF AGRICULTURE";
  doc.setFontSize(13);
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[220,252,231], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });

  doc.save("CA_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
