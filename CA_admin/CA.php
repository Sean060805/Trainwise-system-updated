<?php
/* =============================================================================
   LSPU — COLLEGE OF AGRICULTURE (CA) DEPARTMENT ADMIN DASHBOARD
   -----------------------------------------------------------------------------
   ...
   keeping CA.php's original backend logic (faculty stats, evaluation
...
/* -----------------------------------------------------------------------------
   TRAINING DEMAND FORWARDED TO CA — HR's demand-aggregation pipeline
   ...
   Only demand HR has forwarded, with at least one CA requester.
   2026-09-03 - ported into this redesigned dashboard from the pre-redesign
   CA.php, which had this feature but this file's visual backbone (from
...
// canonicalTnaCollegeCode() rather than an exact 'CA' string match, so
...
// Only show demands where CA actually has a requester (unchanged gating) -
// but show the TRUE cross-college total, not just CA's own count, so the
...
/* =============================================================================
   NEW · ASSESSMENT FORM ANALYTICS (CA department)
...
try {
    ...
    if ($activeDeadline) {
        ...
        $eligibleRes = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = 'CA' AND status = 'accepted'");
        ...
        $stmt = $con->prepare("
            ...
            WHERE u.department = 'CA' AND u.status = 'accepted'
    ...
    error_log("Assessment Form analytics error in CA.php: " . $e->getMessage());
}

/* =============================================================================
   NEW · IDP (INDIVIDUAL DEVELOPMENT PLAN) FORM ANALYTICS — CA department
...
if ($idpTableCheck && $idpTableCheck->num_rows > 0) {
    ...
    try {
        ...
        error_log("IDP Form analytics error in CA.php: " . $e->getMessage());
    }
}
...
$deanNotif = getDeanNotifications($con, $user_id, 'CA');
...
<head>
  ...
  <title>CA Admin Dashboard · LSPU TNA</title>
  ...
  <link rel="stylesheet" href="../assets/css/tw-41.css">

  <!-- ===========================================================
       DESIGN SYSTEM — copied 1:1 from admin_page.php so both dashboards
       share the exact same look. CA-only additions are appended at the
       bottom under "CA EXTRAS".
       =========================================================== -->
  <style>
    :root {
      --bg:          #f5f6f8;
      --bg-grad:     radial-gradient(1100px 600px at 100% -8%, rgba(22,163,74,0.05), transparent 60%);
      --surface:     #ffffff;
      --surface-2:   #f8fafc;
      --surface-3:   #f1f3f7;
      --ink:         #0f172a;
    ...
    *, *::before, *::after { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .avatar { width: 34px; height: 34px; border-radius: 9px; flex-shrink: 0; background: linear-gradient(135deg, var(--accent), #4ade80); color: #fff; font-weight: 700; font-size: 0.85rem; display: flex; align-items: center; justify-content: center; font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .badge-brown    { background: var(--brown-soft); color: #854d0e; }
    ...
    /* ===========================================================
       CA EXTRAS — kept from the original CA.php for the faculty
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/ca.png" alt="CA" class="rail-logo" style="width:32px;height:32px;padding:3px;"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="CA.php" class="rail-link active">
      ...
      <a href="CA_Assessment Form.php" class="rail-link">
          ...
          <div class="dropdown-panel">
            ...
            <div id="notifDropdownList">
              <?php if (!empty($notifications)): ?>
                ...
                ?>
                  <div class="dropdown-item"<?= $isUnread ? ' style="background:var(--accent-soft, #dcfce7);"' : '' ?>>
        ...
        <div class="dropdown" id="avatarDropdown">
          ...
          <button class="avatar-chip" onclick="toggleDropdown('avatarDropdown')" aria-label="Account menu">
            ...
            <span class="avatar-meta">
              ...
              <span class="rl">CA Dean / Admin</span>
            </span>
            ...
            <div class="dropdown-head">
              <div style="display:flex;align-items:center;gap:0.65rem;">
                ...
                <div style="line-height:1.2;">
                  ...
                  <div style="font-size:0.72rem;color:var(--muted);">College of Agriculture</div>
                </div>
              </div>
            </div>
    ...
    <div class="content-scroll">
      <div class="content">
        ...
        <section class="grid-2col">
            ...
            <div class="card-body">
              ...
              <?php else: ?>
                <div class="empty compact">
                  ...
                  <p>Once the main admin sets an active TNA deadline, submission analytics for CA will appear here.</p>
                </div>
              <?php endif; ?>
            </div>
          </div>
          ...
          <div class="card hoverable">
            <div class="card-head">
              ...
              <div class="card-head-icon" style="background:var(--brown-soft);color:#854d0e;"><i class="ri-pie-chart-line"></i></div>
            </div>
            <div class="card-body">
              ...
              <?php if (!empty($unevaluated_faculty)): ?>
                    ...
                    <div class="evaluation-row">
                      <div class="evaluation-info">
                        ...
                        <div class="evaluation-meta">
                          <span class="evaluation-meta-item"><i class="fas fa-building" style="color:var(--accent);"></i> College of Agriculture</span>
                      ...
                      <div class="evaluation-actions">
                        <a href="ca_eval.php?user_id=<?= $faculty['id'] ?>" class="btn btn-green btn-sm">Evaluate Now</a>
                      </div>
                    </div>
                  <?php endforeach; ?>
                </div>
                ...
                <?php if (count($unevaluated_faculty) > 5): ?>
                  <div style="margin-top:1rem;text-align:center;">
                    <a href="ca_eval.php" class="btn btn-ghost btn-sm">
        ...
        <!-- ===========================================================
             ...
             Report to HR modal now live in CA_Training_Demand.php.
             =========================================================== -->
        <a href="CA_Training_Demand.php" style="display:block;margin-top:1.15rem;">
    ...
    <div class="bg-white rounded-2xl shadow-2xl modal-container w-full max-w-6xl">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="mt-8 flex justify-end space-x-4 pt-6 border-t" style="border-color:var(--line);">
                ...
                <?php endif; ?>
                <a href="ca_eval.php?evaluation_id=<?= $evaluation_details['id'] ?>&user_id=<?= $evaluation_details['user_id'] ?>" class="btn btn-primary"><i class="ri-edit-line"></i>Edit Evaluation</a>
            </div>
        </div>
    </div>
</div>
...
function updateNotifications() {
  ...
  // which 404s from inside CA_admin/ since the endpoint lives at the
    ...
    .then(data => {
      ...
      if (items.length > 0) {
        ...
        items.forEach(notification => {
          ...
          if (isUnread) item.style.background = '#dcfce7';
          item.innerHTML = `
      ...
      // Keep the bell-dot/badge in sync with each poll too, not just the
      // initial page load and the optimistic clear in markAllAsRead().
    ...
    new Chart(progressEl.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#059669', '#a16207', '#16a34a'],
    ...
    new Chart(distEl.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['rgba(5,150,105,0.85)', 'rgba(161,98,7,0.85)'],
          borderColor: ['#059669', '#a16207'],
