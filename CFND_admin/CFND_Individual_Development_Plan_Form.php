<?php
/* ===================================================================
   CFND IDP SUBMISSIONS — Department view for College of Food Nutrition and Dietetics
   =================================================================== */
...
$forwardedDemandCount = getForwardedDemandCountForCollege($con, 'CFND');
...
function processSignatureUpload($file, $role, $formId)
{
    ...
    $uploadDir = '../uploads/signatures/cfnd/';
...
/*
|--------------------------------------------------------------------------
| Main employee list - FILTERED TO CFND DEPARTMENT
|--------------------------------------------------------------------------
*/
$query = "
    SELECT 
    ...
    WHERE f.status = 'submitted' AND u.department = 'CFND'
...
$statsResult = $con->query("
    ...
    FROM users 
    WHERE department = 'CFND' AND role = 'user' 
    AND teaching_status IS NOT NULL AND teaching_status != ''
");
if ($statsResult) {
    $cfndStats['total_employees'] = $statsResult->fetch_assoc()['count'];
}
$cfndStats['with_idp'] = count($employees);
...
$hrResult = $con->query("
    ...
    FROM idp_forms f
    JOIN users u ON f.user_id = u.id
    WHERE f.status = 'submitted_to_hr' AND u.department = 'CFND'
");
if ($hrResult) {
    $cfndStats['submitted_to_hr'] = $hrResult->fetch_assoc()['count'];
}
...
<head>
  ...
  <title>CFND IDP Submissions · LSPU TNA</title>
  ...
  <!-- TAILWIND -->
  <link rel="stylesheet" href="../assets/css/tw-21.css">
<style>
  /* ==========================================================
     DESIGN SYSTEM — CFND Pink Accent Version
     ========================================================== */
  ...
  :root {
    ...
    --accent:#ec4899; --accent-700:#db2777; --accent-soft:#fce7f3; --accent-ink:#9d174d;
    --pink:#ec4899; --pink-light:#f472b6; --pink-soft:#fce7f3;
    --white:#ffffff; --white-soft:#fdf2f8;
    ...
    --rail:#2d0a1a; --rail-2:#4d1a2e; --rail-line:rgba(148,163,184,0.14);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .date-chip i { color:var(--pink); font-size:1.05rem; }
  ...
  .stat::after { content:''; position:absolute; inset:0 0 auto 0; height:3px; background:var(--stat-accent, var(--pink)); }
  .stat.a-pink   { --stat-accent:linear-gradient(90deg, var(--pink-light), var(--pink)); }
  ...
  .ico-pink   { background:var(--pink-soft); color:#9d174d; }
  ...
  .btn-pink { background:var(--pink); color:#fff; box-shadow:0 8px 18px -10px rgba(236,72,153,0.6); }
  ...
  .view-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--pink); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.85rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(236,72,153,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
  .view-btn:hover { background:#db2777; }
  ...
  .department-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--pink-soft); color:#9d174d; padding:0.32rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .emp-avatar { width:38px; height:38px; border-radius:10px; background:var(--pink-soft); color:#9d174d; display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .search input:focus { outline:none; border-color:var(--pink); box-shadow:0 0 0 4px rgba(236,72,153,0.12); }
  ...
  .search-go { position:absolute; right:6px; top:50%; transform:translateY(-50%); width:32px; height:32px; border:none; border-radius:8px; background:var(--pink); color:#fff; cursor:pointer; display:inline-flex; align-items:center; justify-content:center; transition:background 0.18s var(--ease); }
  .search-go:hover { background:#db2777; }
  ...
  .filter.open .filter-btn { border-color:var(--pink); }
  ...
  .status-select:focus { outline:none; border-color:var(--pink); box-shadow:0 0 0 4px rgba(236,72,153,0.12); }
  ...
  .modal-table tr:hover td { background:var(--pink-soft); }
  .modal-table tr:last-child td { border-bottom:none; }
  .checkbox-custom { width:18px; height:18px; accent-color:var(--pink); }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="CFND.php" class="rail-link">
      ...
      <a href="CFND_Individual_Development_Plan_Form.php" class="rail-link active">
  ...
  <div class="app-main">
    <header class="topbar">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip" aria-label="Account">
          ...
          <span class="avatar-meta"><span class="nm"><?= htmlspecialchars($adminName) ?></span> <span class="rl">CFND Admin</span></span>
        </div>
      </div>
    </header>
    ...
    <div class="content-scroll">
      <div class="content">
        <div class="page-head">
          <div>
            <p class="eyebrow">Individual Development Plans · CFND</p>
            <h2>CFND IDP Submissions</h2>
            <p>View and manage IDP forms submitted by College of Food Nutrition and Dietetics faculty and staff.</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">Total CFND Faculty</p>
                <p class="stat-value num"><?php echo $cfndStats['total_employees']; ?></p>
          ...
          <div class="stat a-pink">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $cfndStats['with_idp']; ?></p>
                <p class="stat-sub"><?php echo $cfndStats['total_employees'] > 0 ? round(($cfndStats['with_idp'] / $cfndStats['total_employees']) * 100, 1) : 0; ?>% of faculty</p>
              </div>
              <div class="stat-ico ico-pink"><i class="ri-file-list-3-line"></i></div>
            </div>
          </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $cfndStats['submitted_to_hr']; ?></p>
        ...
        <div class="card" style="margin-bottom:1.4rem; overflow:visible; position:relative; z-index:20;">
          <div class="card-body" style="padding:1.05rem 1.15rem;">
            <div class="filterbar">
              <div style="display:inline-flex;align-items:center;gap:0.55rem;height:42px;padding:0 0.9rem;border:1px solid var(--pink);border-radius:var(--radius-sm);background:var(--pink-soft);">
                <i class="ri-building-line" style="color:#9d174d;"></i>
                <span style="font-weight:600;color:#9d174d;font-size:0.85rem;">CFND</span>
                <span style="font-size:0.7rem;color:#9d174d;opacity:0.7;">College of Food Nutrition & Dietetics</span>
              </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p style="font-size:0.82rem; color:var(--muted);">
            ...
            of <span class="num"><?= count($employees) ?></span> CFND employees with submitted IDPs
          </p>
        </div>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No IDP submissions found for CFND</h4>
                        <p>CFND faculty and staff will appear here once they submit their IDP forms.</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($employees as $employee): ?>
                    ...
                    <tr class="employee-item" data-department="<?= htmlspecialchars($employee['department']) ?>">
                      ...
                      <td><span class="num" style="font-weight:700; color:var(--pink); font-size:1.05rem;"><?= (int) $employee['total_idps'] ?></span></td>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="signature-upload-card">
                    ...
                    <input
                      ...
                      class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-pink file:text-white hover:file:bg-[#db2777] cursor-pointer signature-file-input"
  ...
  function showIDPModal(userId, userName, expandFormId = null) {
    ...
    modalContent.innerHTML = `
      <div class="flex justify-center items-center" style="padding:3rem 0;">
        <div style="width:36px;height:36px;border:3px solid var(--pink-soft);border-top-color:var(--pink);border-radius:50%;animation:spin 0.8s linear infinite;"></div>
      </div>
