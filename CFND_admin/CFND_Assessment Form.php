<?php
/* ===================================================================
   CFND — ASSESSMENT FORM SUBMISSIONS (College of Food Nutrition and Dietetics only)
   -------------------------------------------------------------------
   This is a CFND-scoped copy of "Assessment Form.php". EVERY query below
   is constrained to department = 'CFND' — a CFND admin can only ever see
   or act on CFND records. UI matches CFND.php (dark rail sidebar, pink
   accent, dual LSPU + CFND logos).

   Assumed to live in the same folder as CFND.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as CFND.php — keeps this page inside the protected CFND area.
...
// ----- Delete handler (POST) — only allowed for CFND-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a CFND user.
        $cfndDept = CFND_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $cfndDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — CFND only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . CFND_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — CFND only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . CFND_DEPT . "'";
...
// ----- Count total rows (respects active filters, CFND only) ---------
...
// ----- Main paginated query (sorted by surname, CFND only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all CFND rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted CFND faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always CFND).
$cfnd_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . CFND_DEPT . "' AND status = 'accepted'");
$cfnd_eligible = ($cfnd_eligible_result && $cfnd_eligible_result->num_rows > 0) ? (int)$cfnd_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $cfnd_eligible > 0 ? round(($total_rows / $cfnd_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>CFND Assessment Forms · LSPU TNA</title>
...
<style>
  /* Design system — same tokens as CFND.php / admin_page.php, pink-accented rail. */
  :root {
    --bg:#f5f6f8; --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(236,72,153,0.05), transparent 60%);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:32px; height:32px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--pink), var(--pink-light)); display:flex; align-items:center; justify-content:center; }
  ...
  .scope-badge { display:inline-flex; align-items:center; gap:0.4rem; background:var(--pink-soft); color:#9d174d; padding:0.3rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; margin-left:0.5rem; vertical-align:middle; }
  ...
  .btn-white { background:var(--white); color:#9d174d; border:1px solid #fce7f3; box-shadow:0 8px 18px -10px rgba(255,255,255,0.55); }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--pink-soft); color:#9d174d; padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .search input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(236,72,153,0.12); }
  ...
  .filter-menu a.sel { background:var(--pink-soft); color:#9d174d; font-weight:600; }
  ...
  .pg.active { background:var(--pink); color:#fff; border-color:var(--pink); }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/cfnd.png" alt="CFND" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="CFND_Assessment Form.php" class="rail-link active">
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>CFND Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> CFND only</span></h2>
            <p>Viewing submissions from College of Food Nutrition and Dietetics faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
          <button onclick="generatePDF()" class="btn btn-pink">
        ...
        <div class="grid-stats">
          <div class="stat a-pink">
            <div class="stat-body">
              <div>
                <p class="stat-label">CFND Records</p>
          ...
          <div class="stat a-white">
            <div class="stat-body">
              ...
              <div class="stat-ico ico-white"><i class="ri-user-line"></i></div>
            </div>
          </div>
          <div class="stat a-amber">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $cfnd_eligible ?> CFND accepted users</p>
              </div>
        ...
        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to CFND) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> CFND records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Food Nutrition and Dietetics</div>
                      </td>
                      <td>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No CFND submissions found</h4>
                        <p>
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Food Nutrition and Dietetics</p></div>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF FOOD NUTRITION AND DIETETICS";
  doc.setFontSize(13);
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[252,231,243], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });

  doc.save("CFND_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
