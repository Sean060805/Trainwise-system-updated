<?php
/* ===================================================================
   COF — ASSESSMENT FORM SUBMISSIONS (College of Fisheries only)
   -------------------------------------------------------------------
   This is a COF-scoped copy of "Assessment Form.php". EVERY query below
   is constrained to department = 'COF' — a COF admin can only ever see
   or act on COF records. UI matches COF.php (dark rail sidebar, blue
   accent, dual LSPU + COF logos).

   Assumed to live in the same folder as COF.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as COF.php — keeps this page inside the protected COF area.
...
// ----- Delete handler (POST) — only allowed for COF-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a COF user.
        $cofDept = COF_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $cofDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — COF only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . COF_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — COF only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . COF_DEPT . "'";
...
// ----- Count total rows (respects active filters, COF only) ---------
...
// ----- Main paginated query (sorted by surname, COF only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all COF rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted COF faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always COF).
$cof_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . COF_DEPT . "' AND status = 'accepted'");
$cof_eligible = ($cof_eligible_result && $cof_eligible_result->num_rows > 0) ? (int)$cof_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $cof_eligible > 0 ? round(($total_rows / $cof_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>COF Assessment Forms · LSPU TNA</title>
...
<style>
  /* Design system — same tokens as COF.php / admin_page.php, blue-accented rail. */
  :root {
    --bg:#f5f6f8; --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(37,99,235,0.05), transparent 60%);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:32px; height:32px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--blue), var(--blue-light)); display:flex; align-items:center; justify-content:center; }
  ...
  .scope-badge { display:inline-flex; align-items:center; gap:0.4rem; background:var(--blue-soft); color:#1e3a8a; padding:0.3rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; margin-left:0.5rem; vertical-align:middle; }
  ...
  .btn-dark { background:var(--dark); color:#fff; box-shadow:0 8px 18px -10px rgba(31,41,55,0.55); }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--blue-soft); color:#1e3a8a; padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .search input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(37,99,235,0.12); }
  ...
  .filter-menu a.sel { background:var(--blue-soft); color:#1e3a8a; font-weight:600; }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/cof-logo.png" alt="COF" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="COF_Assessment Form.php" class="rail-link active">
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>COF Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> COF only</span></h2>
            <p>Viewing submissions from College of Fisheries faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
          <button onclick="generatePDF()" class="btn btn-blue">
        ...
        <div class="grid-stats">
          <div class="stat a-blue">
            <div class="stat-body">
              <div>
                <p class="stat-label">COF Records</p>
          ...
          <div class="stat a-dark">
            <div class="stat-body">
              ...
              <div class="stat-ico ico-dark"><i class="ri-user-line"></i></div>
            </div>
          </div>
          <div class="stat a-amber">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $cof_eligible ?> COF accepted users</p>
              </div>
        ...
        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to COF) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> COF records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Fisheries</div>
                      </td>
                      <td>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No COF submissions found</h4>
                        <p>
...
<!-- HIDDEN EXPORT TABLE (COF rows only) -->
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Fisheries</p></div>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF FISHERIES";
  doc.setFontSize(13);
  ...
  doc.save("COF_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
