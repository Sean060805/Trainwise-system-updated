<?php
/* =============================================================================
   LSPU — COLLEGE OF CRIMINAL JUSTICE EDUCATION (CCJE) DEPARTMENT ADMIN DASHBOARD
   -----------------------------------------------------------------------------
   2026-09-04 - full rebuild. CCJE was the only college dashboard still on the
   pre-redesign template (found during a "replicate the other colleges'
   changes" request) - this brings it in line with CA.php/CAS.php's actual
   current structure (rail sidebar, topbar with notification+avatar
   dropdowns, Assessment Form + IDP Forms analytics cards, 4 charts) rather
   than the plain color-swap this dashboard got in an earlier, incomplete
   pass. CCJE keeps its OWN navy/gold accent (CA already owns green) and its
   OWN correct business logic throughout - see the inline notes below for
   what was deliberately preserved rather than copied from CA/CAS:
     - The evaluation-detail modal ("TRAINING PROGRAM IMPACT ASSESSMENT
       FORM" / "IMPACT/BENEFITS GAINED" criteria) - CA's own modal shows
       DIFFERENT, mismatched criteria that don't match what this app's real
       submission form (training_program_impact_assessment_form.php)
       actually collects. Copying CA's modal wholesale would have replaced
       CCJE's correct content with CA's wrong content.
     - The extra `training_demand`-sourced notification (CCJE already had
       this; CA's dashboard doesn't).
     - The evaluation stats queries (already fixed 2026-09-03 to be
       properly department-scoped - CA's own version was the reference for
       that fix, so they now match).
   ============================================================================= */
...
/* -----------------------------------------------------------------------------
   TRAINING DEMAND FORWARDED TO CCJE — HR's demand-aggregation pipeline
   (preserved from the pre-rebuild CCJE.php - already correct, already
   handles cross-college totals via canonicalTnaCollegeCode()).
   ----------------------------------------------------------------------------- */
...
try {
    ...
    // 2026-09-03 fix (preserved) - department-scoped, was silently counting
    // every college's evaluations before that fix.
    $evaluation_stats_result = $con->query("
        ...
        FROM evaluations
        WHERE EXISTS (
            SELECT 1 FROM users u WHERE u.id = evaluations.user_id AND u.department = 'CCJE'
        )
    ");
    ...
    $recent_result = $con->query("
        SELECT u.id, u.name, u.department, u.teaching_status, e.id as evaluation_id, e.status as evaluation_status, e.created_at
        FROM evaluations e
        JOIN users u ON e.user_id = u.id
        WHERE u.department = 'CCJE'
...
/* =============================================================================
   ASSESSMENT FORM ANALYTICS (CCJE department) - new, ported from CA.php/
   admin_page.php. Reuses the same assessments + settings tables.
   ============================================================================= */
$assessmentStats = [
    'has_active_deadline' => false, 'active_title' => null, 'active_deadline' => null,
    'total_eligible' => 0, 'on_time' => 0, 'late' => 0, 'no_submission' => 0,
    'total_submitted' => 0, 'submission_rate' => 0.0,
];
try {
    ...
    if ($activeDeadline) {
        ...
        $eligibleRes = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = 'CCJE' AND status = 'accepted'");
        ...
        $stmt = $con->prepare("
            ...
            WHERE u.department = 'CCJE' AND u.status = 'accepted'
        ...
        $assessmentStats['submission_rate'] = $assessmentStats['total_eligible'] > 0
            ? round(($assessmentStats['total_submitted'] / $assessmentStats['total_eligible']) * 100, 1) : 0.0;
    }
} catch (Exception $e) {
    error_log("Assessment Form analytics error in CCJE.php: " . $e->getMessage());
}

/* =============================================================================
   IDP FORM ANALYTICS (CCJE department) - new, ported from CA.php. Fails
   safely to an empty state if idp_forms doesn't exist.
   ============================================================================= */
$idpStats = [
    'table_available' => false, 'total_eligible' => 0, 'draft' => 0, 'submitted' => 0,
    'approved' => 0, 'rejected' => 0, 'total_created' => 0, 'no_idp' => 0, 'completion_rate' => 0.0,
];
...
if ($idpTableCheck && $idpTableCheck->num_rows > 0) {
    ...
    try {
        ...
        $idpStats['completion_rate'] = $idpStats['total_eligible'] > 0
            ? round(($idpStats['approved'] / $idpStats['total_eligible']) * 100, 1) : 0.0;
    } catch (Exception $e) {
        error_log("IDP Form analytics error in CCJE.php: " . $e->getMessage());
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_evaluation'])) {
    ...
    try {
        ...
        if ($stmt->affected_rows > 0) {
            $workflow_stmt = $con->prepare("INSERT INTO evaluation_workflow (evaluation_id, from_status, to_status, changed_by) VALUES (?, 'draft', 'submitted', ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['view_evaluation'])) {
    ...
    try {
        $stmt = $con->prepare("
            SELECT e.*, u.name as employee_name, u.department as employee_department, evaluator.name as evaluator_name
            FROM evaluations e
            JOIN users u ON e.user_id = u.id
        ...
        if ($evaluation_result->num_rows > 0) {
            ...
            $ratings_stmt = $con->prepare("SELECT * FROM evaluation_ratings WHERE evaluation_id = ? ORDER BY question_number");
            ...
            $workflow_stmt = $con->prepare("
                SELECT wf.*, u.name as changed_by_name FROM evaluation_workflow wf
                JOIN users u ON wf.changed_by = u.id WHERE wf.evaluation_id = ? ORDER BY wf.created_at ASC
            ");
...
// getDeanNotifications() in ml_recommendations.php for why the badge
// count and "Mark all as read" now behave correctly).
$deanNotif = getDeanNotifications($con, $user_id, 'CCJE');
...
<head>
  ...
  <title>CCJE Admin Dashboard · LSPU TNA</title>
  ...
  <link rel="stylesheet" href="../assets/css/tw-37.css">

  <!-- Design system - same structure as CA.php/CAS.php, navy/gold accent for CCJE -->
  <style>
    :root {
      --bg: #f5f6f8;
      --bg-grad: radial-gradient(1100px 600px at 100% -8%, rgba(30,58,95,0.05), transparent 60%);
      ...
      --accent: #2c5282; --accent-600: #2c5282; --accent-700: #1e3a5f; --accent-soft: #dbeafe; --accent-ink: #1e3a5f;
      ...
      --green: #1e3a5f; --green-soft: #dbeafe;
      --brown: #92702a; --brown-soft: #fef3c7;
    }
    ...
    *, *::before, *::after { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .rail-logo-fallback { width: 38px; height: 38px; border-radius: 10px; flex-shrink: 0; background: linear-gradient(135deg, var(--green), var(--accent)); display: flex; align-items: center; justify-content: center; }
    ...
    .avatar { width: 34px; height: 34px; border-radius: 9px; flex-shrink: 0; background: linear-gradient(135deg, var(--accent), var(--green-light, #4a7ba6)); color: #fff; font-weight: 700; font-size: 0.85rem; display: flex; align-items: center; justify-content: center; font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .badge-green { background: var(--green-soft); color: #1e3a5f; }
    ...
    .modal-backdrop { position: fixed; inset: 0; z-index: 60; backdrop-filter: blur(8px); background: rgba(15,23,42,0.6); animation: backdropFadeIn 0.25s ease forwards; opacity: 0; }
    ...
    /* Evaluation grid + evaluation-detail modal + notification-item -
       preserved from the pre-rebuild CCJE.php, reskinned to the tokens
       above. */
    ...
    .status-submitted { background: #dbeafe; color: #1e3a5f; }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/ccje.png" alt="CCJE" class="rail-logo" style="width:32px;height:32px;padding:3px;"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="CCJE.php" class="rail-link active">
      ...
      <a href="CCJE_Assessment Form.php" class="rail-link">
          ...
          <div class="dropdown-panel">
            ...
            <div id="notifDropdownList">
              <?php if (!empty($notifications)): ?>
                ...
                ?>
                  <div class="dropdown-item"<?= $isUnread ? ' style="background:var(--accent-soft, #dbeafe);"' : '' ?>>
        ...
        <div class="dropdown" id="avatarDropdown">
          ...
          <button class="avatar-chip" onclick="toggleDropdown('avatarDropdown')" aria-label="Account menu">
            ...
            <span class="avatar-meta">
              ...
              <span class="rl">CCJE Dean / Admin</span>
            </span>
            ...
            <div class="dropdown-head">
              <div style="display:flex;align-items:center;gap:0.65rem;">
                ...
                <div style="line-height:1.2;">
                  ...
                  <div style="font-size:0.72rem;color:var(--muted);">College of Criminal Justice Education</div>
                </div>
              </div>
            </div>
    ...
    <div class="content-scroll">
      <div class="content">
          ...
          <div class="stat a-green">
            <div class="stat-body">
              <div>
                ...
                <h3 class="stat-value num" style="color:var(--ink);"><?= $stats['total_employees'] ?></h3>
              </div>
          ...
          <div class="stat a-brown">
            <div class="stat-body">
              <div>
                ...
                <h3 class="stat-value num" style="color:var(--brown);"><?= $stats['non_teaching'] ?></h3>
              </div>
              <div class="stat-ico ico-brown"><i class="fas fa-user-tie"></i></div>
            </div>
          </div>
        ...
        <section class="grid-2col">
            ...
            <div class="card-body">
              ...
              <?php else: ?>
                <div class="empty compact">
                  ...
                  <p>Once the main admin sets an active TNA deadline, submission analytics for CCJE will appear here.</p>
                </div>
              <?php endif; ?>
            </div>
          </div>
          ...
          <div class="card hoverable">
            <div class="card-head">
              ...
              <div class="card-head-icon" style="background:var(--brown-soft);color:#6b5219;"><i class="ri-pie-chart-line"></i></div>
            </div>
            <div class="card-body">
              ...
              <?php else: ?>
                <div class="empty compact">
                  ...
                  <p>This dashboard expects an <code>idp_forms</code> table (id, user_id, status, created_at).</p>
                </div>
              <?php endif; ?>
            </div>
          </div>
        ...
        <section class="grid-main">
          <div class="card hoverable">
            <div class="card-head">
              ...
              <span class="badge badge-green num"><?= count($unevaluated_employees) ?> pending</span>
            </div>
            <div class="card-body">
              <?php if (!empty($unevaluated_employees)): ?>
                    ...
                    <div class="evaluation-row">
                      <div class="evaluation-info">
                        ...
                        <div class="evaluation-meta">
                          <span class="evaluation-meta-item"><i class="fas fa-building" style="color:var(--accent);"></i> College of Criminal Justice Education</span>
                          <span class="evaluation-meta-item">
                            ...
                            <?php else: ?>
                              <i class="fas fa-user-tie" style="color:var(--brown);"></i> Non-Teaching
                            <?php endif; ?>
                          </span>
                        </div>
                      </div>
                      <div class="evaluation-actions">
                        <a href="ccje_eval.php?user_id=<?= $employee['id'] ?>" class="btn btn-green btn-sm">Evaluate Now</a>
                      </div>
                    </div>
                  <?php endforeach; ?>
                </div>
                <?php if (count($unevaluated_employees) > 5): ?>
                  <div style="margin-top:1rem;text-align:center;">
                    <a href="ccje_eval.php" class="btn btn-ghost btn-sm">
                      View all <?= count($unevaluated_employees) ?> pending evaluations <i class="ri-arrow-right-line"></i>
                    </a>
                  </div>
                <?php endif; ?>
              <?php else: ?>
        ...
        <!-- Training Demand - slim teaser card linking to the dedicated
             ...
             Report to HR modal now live in CCJE_Training_Demand.php. -->
        <a href="CCJE_Training_Demand.php" style="display:block;margin-top:1.15rem;">
...
<!-- Evaluation View Modal - preserved from the pre-rebuild CCJE.php:
     "TRAINING PROGRAM IMPACT ASSESSMENT FORM" / "IMPACT/BENEFITS GAINED"
     criteria (matches the real submission form). Deliberately NOT copied
     from CA.php - CA's own modal shows different, mismatched criteria. -->
<?php if ($show_evaluation_modal && $evaluation_details): ?>
<div class="modal-backdrop">
  <div class="bg-white rounded-2xl shadow-2xl modal-container w-full max-w-6xl" style="margin:2rem auto;">
    ...
    <div class="p-6 space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div>
          ...
          <div class="w-full p-3 rounded-lg border" style="background:var(--surface-2);border-color:var(--line);"><?= htmlspecialchars($evaluation_details['training_title']) ?></div>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Date Conducted:</label>
          <div class="w-full p-3 rounded-lg border" style="background:var(--surface-2);border-color:var(--line);"><?= date('M d, Y', strtotime($evaluation_details['date_conducted'])) ?></div>
        </div>
      </div>

      <div class="mb-6">
        <label class="block text-sm font-medium text-gray-700 mb-1">Objective/s:</label>
        <div class="w-full p-3 rounded-lg border min-h-[80px]" style="background:var(--surface-2);border-color:var(--line);"><?= nl2br(htmlspecialchars($evaluation_details['objectives'])) ?></div>
      </div>

      <div class="mb-6">
        <div class="p-3 rounded mb-4" style="background:var(--surface-2);">
          ...
          <table class="w-full border-collapse">
            <thead>
              <tr style="background:var(--accent);">
                <th class="text-left py-3 px-4 font-medium text-white w-1/2">IMPACT/BENEFITS GAINED</th>
                ...
                <th class="text-left py-3 px-4 font-medium text-white">REMARKS</th>
              </tr>
            </thead>
            <tbody>
              <?php
              ...
              ?>
              <tr class="<?= $i % 2 === 0 ? '' : 'bg-white' ?>" <?= $i % 2 === 0 ? 'style="background:var(--surface-2);"' : '' ?>>
                <td class="py-3 px-4 text-gray-700 text-sm" style="border:1px solid var(--line);"><?= $questions[$i-1] ?></td>
                <?php for ($j = 1; $j <= 5; $j++): ?>
                <td class="text-center py-2 px-1" style="border:1px solid var(--line);">
                  <?php if ($rating && $rating['rating'] == $j): ?>
                    <div class="w-8 h-8 flex items-center justify-center mx-auto text-white rounded" style="background:var(--green);"><i class="ri-check-line text-sm"></i></div>
                  <?php else: ?>
      ...
      <div class="mt-8 flex justify-end space-x-4 pt-6 border-t" style="border-color:var(--line);">
        ...
        <?php endif; ?>
        <a href="ccje_eval.php?evaluation_id=<?= $evaluation_details['id'] ?>&user_id=<?= $evaluation_details['user_id'] ?>" class="btn btn-primary"><i class="ri-edit-line"></i>Edit Evaluation</a>
      </div>
    </div>
  </div>
</div>
...
/* 2026-09-06 fix - this dashboard was rebuilt earlier this session with
   its own toast/dropdown JS and the notification-bell's "Mark all as
   read" button + its poll/mark-read functions were dropped in the
   process; the button never existed at all, let alone worked. Added
   back here matching the pattern every other college dashboard uses
   (with the correct relative paths, since this file lives in
   CCJE_admin/ and the endpoints live at the project root). See
   CLAUDE.md. */
    ...
    .then(data => {
      ...
      if (items.length > 0) {
        ...
        items.forEach(notification => {
          ...
          if (isUnread) item.style.background = '#dbeafe';
          item.innerHTML = `
    ...
    new Chart(progressEl.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#059669', '#d97706', '#2c5282'],
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: { label(c) { const total = c.dataset.data.reduce((a, b) => a + b, 0); const p = total > 0 ? Math.round((c.raw / total) * 100) : 0; return ` ${c.label}: ${c.raw} (${p}%)`; } }
          }
        }
      }
    });
    ...
    new Chart(distEl.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['rgba(5,150,105,0.85)', 'rgba(146,112,42,0.85)'],
          borderColor: ['#059669', '#92702a'],
  ...
  if (idpEl && typeof Chart !== 'undefined') {
    new Chart(idpEl.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          label: 'Faculty',
          data: [<?= (int)$idpStats['no_idp'] ?>, <?= (int)$idpStats['draft'] ?>, <?= (int)$idpStats['submitted'] ?>, <?= (int)$idpStats['approved'] ?>, <?= (int)$idpStats['rejected'] ?>],
...
<?php if (isset($_GET['success'])): ?>
  Swal.fire({ icon: 'success', title: 'Success!', text: 'Evaluation submitted to HR successfully!', confirmButtonColor: '#2c5282', background: '#fff', color: '#16283f', confirmButtonText: 'Continue', timer: 3000, timerProgressBar: true });
<?php elseif (isset($error_message) && !empty($error_message)): ?>
  Swal.fire({ icon: 'error', title: 'Error', text: '<?= addslashes($error_message) ?>', confirmButtonColor: '#2c5282', background: '#fff', color: '#16283f' });
<?php endif; ?>
</script>
</body>
</html>
