<?php
/* ===================================================================
   CCJE IDP SUBMISSIONS — Department view for College of Criminal
   Justice Education
   ...
   it (every other college already had one). Adapted from
   CA_admin/CA_Individual_Development_Plan_Form.php - department scope
   swapped to CCJE, signature upload path swapped to uploads/signatures/
   ccje/, accent color swapped to navy/gold (matches the same choice made
   for CCJE_Assessment Form.php - CA already owns green, navy+gold fits
   a justice-education college and doesn't collide with any other
   college's accent).
   =================================================================== */
...
function processSignatureUpload($file, $role, $formId)
{
    ...
    $uploadDir = '../uploads/signatures/ccje/';
...
/*
|--------------------------------------------------------------------------
| Main employee list - FILTERED TO CA DEPARTMENT
|--------------------------------------------------------------------------
*/
$query = "
    SELECT 
    ...
    WHERE f.status = 'submitted' AND u.department = 'CCJE'
...
$statsResult = $con->query("
    ...
    FROM users
    WHERE department = 'CCJE' AND role = 'user'
    AND teaching_status IS NOT NULL AND teaching_status != ''
");
if ($statsResult) {
    $caStats['total_employees'] = $statsResult->fetch_assoc()['count'];
}
$caStats['with_idp'] = count($employees);
...
$hrResult = $con->query("
    ...
    FROM idp_forms f
    JOIN users u ON f.user_id = u.id
    WHERE f.status = 'submitted_to_hr' AND u.department = 'CCJE'
");
if ($hrResult) {
    $caStats['submitted_to_hr'] = $hrResult->fetch_assoc()['count'];
}
...
<head>
  ...
  <title>CCJE IDP Submissions · LSPU TNA</title>
...
<style>
  /* ==========================================================
     DESIGN SYSTEM — CA Green Accent Version
     ========================================================== */
  ...
  :root {
    ...
    --accent:#1e3a5f; --accent-700:#16283f; --accent-soft:#dbeafe; --accent-ink:#1e3a5f;
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-link.active { background:linear-gradient(100deg, rgba(30,58,95,0.26), rgba(30,58,95,0.08)); color:#fff; border-color:rgba(74,123,166,0.35); }
  ...
  .btn-primary { background:var(--accent); color:#fff; box-shadow:0 8px 18px -10px rgba(30,58,95,0.7); }
  ...
  .btn-green { background:var(--green); color:#fff; box-shadow:0 8px 18px -10px rgba(30,58,95,0.6); }
  ...
  .view-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--green); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.85rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(30,58,95,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
  .view-btn:hover { background:#16283f; }
  ...
  .department-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--green-soft); color:#1e3a5f; padding:0.32rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .emp-avatar { width:38px; height:38px; border-radius:10px; background:var(--green-soft); color:#1e3a5f; display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .search input:focus { outline:none; border-color:var(--green); box-shadow:0 0 0 4px rgba(30,58,95,0.12); }
  ...
  .search-go:hover { background:#16283f; }
  ...
  .status-select:focus { outline:none; border-color:var(--green); box-shadow:0 0 0 4px rgba(30,58,95,0.12); }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="CCJE_Individual_Development_Plan_Form.php" class="rail-link active">
  ...
  <div class="app-main">
    <header class="topbar">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip" aria-label="Account">
          ...
          <span class="avatar-meta"><span class="nm"><?= htmlspecialchars($adminName) ?></span> <span class="rl">CCJE Admin</span></span>
        </div>
      </div>
    </header>
    ...
    <div class="content-scroll">
      <div class="content">
        <div class="page-head">
          <div>
            <p class="eyebrow">Individual Development Plans · CCJE</p>
            <h2>CCJE IDP Submissions</h2>
            <p>View and manage IDP forms submitted by College of Criminal Justice Education faculty and staff.</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">Total CA Faculty</p>
                <p class="stat-value num"><?php echo $caStats['total_employees']; ?></p>
          ...
          <div class="stat a-green">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $caStats['with_idp']; ?></p>
                <p class="stat-sub"><?php echo $caStats['total_employees'] > 0 ? round(($caStats['with_idp'] / $caStats['total_employees']) * 100, 1) : 0; ?>% of faculty</p>
              </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $caStats['submitted_to_hr']; ?></p>
          ...
          <div class="card-body" style="padding:1.05rem 1.15rem;">
            <div class="filterbar">
              <div style="display:inline-flex;align-items:center;gap:0.55rem;height:42px;padding:0 0.9rem;border:1px solid var(--green);border-radius:var(--radius-sm);background:var(--green-soft);">
                <i class="ri-building-line" style="color:#1e3a5f;"></i>
                <span style="font-weight:600;color:#1e3a5f;font-size:0.85rem;">CCJE</span>
                <span style="font-size:0.7rem;color:#1e3a5f;opacity:0.7;">College of Criminal Justice Education</span>
              </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p style="font-size:0.82rem; color:var(--muted);">
            ...
            of <span class="num"><?= count($employees) ?></span> CA employees with submitted IDPs
          </p>
        </div>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No IDP submissions found for CA</h4>
                        <p>CCJE faculty and staff will appear here once they submit their IDP forms.</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="signature-upload-card">
                    ...
                    <input
                      ...
                      class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-green file:text-white hover:file:bg-[#16283f] cursor-pointer signature-file-input"
