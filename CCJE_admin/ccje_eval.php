<?php
/* =============================================================================
   CCJE EVALUATION — Department view for College of Criminal Justice Education
   -----------------------------------------------------------------------------
   2026-09-04 - structural rebuild to match ca_eval.php's/cas_eval.php's actual
   current template (rail sidebar + topbar, filter bar, data table, iframe
   modal) - this page was still on the old glass-card/gray-gradient template
   even after CCJE.php's own rebuild earlier today; found while checking
   whether the same gap existed here. CCJE keeps its own navy/gold accent
   (matching CCJE.php), not CA's green.
   ============================================================================= */
...
try {
    // Only show users with non-null teaching_status for CCJE department
    $sql = "SELECT
                u.id AS user_id,
                u.name,
                u.department,
    ...
    error_log("Database error in ccje_eval.php: " . $e->getMessage());
...
<head>
  ...
  <title>CCJE Evaluation · LSPU TNA</title>
  ...
  .rail-badge { margin-left:auto; background:var(--bad); color:#fff; font-size:0.68rem; font-weight:700; padding:0.15rem 0.48rem; border-radius:999px; line-height:1.4; flex-shrink:0; }
    ...
    .avatar { width:34px; height:34px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--accent), var(--green-light)); color:#fff; font-weight:700; font-size:0.85rem; display:flex; align-items:center; justify-content:center; font-family:'Plus Jakarta Sans',sans-serif; }
    ...
    .stat.a-brown   { --stat-accent:linear-gradient(90deg, var(--brown-light), var(--brown)); }
    ...
    .ico-green   { background:var(--green-soft); color:var(--accent-ink); }
    ...
    .evaluate-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--green); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.82rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(30,58,95,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
    .evaluate-btn:hover { background:#16283f; }
    ...
    .badge-submitted { background:#dbeafe; color:#1e3a5f; }
    .badge-approved { background:var(--ok-soft); color:var(--ok-ink); }
    .badge-rejected { background:var(--bad-soft); color:var(--bad-ink); }
    .badge-department { background:var(--green-soft); color:var(--accent-ink); padding:0.32rem 0.7rem; border-radius:999px; font-size:0.7rem; font-weight:700; letter-spacing:0.02em; }
    .badge-teaching { background:#e0f2fe; color:#0369a1; }
    .badge-non-teaching { background:var(--brown-soft); color:#92400e; }
    ...
    .emp-avatar { width:38px; height:38px; border-radius:10px; background:var(--green-soft); color:var(--accent-ink); display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="ccje_eval.php" class="rail-link active">
    ...
    <div style="padding:0 0.7rem 0.7rem;">
      <div class="rail-stats">
        ...
        <div class="rail-stats-item"><span>Evaluated</span><span class="num" style="color:#4a7ba6;"><?= $evaluated_count ?></span></div>
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>CCJE Faculty Evaluations</h2>
            <p>View and manage CCJE faculty training evaluations — <span class="num"><?= $total_faculty ?></span> faculty members</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          ...
          <div class="stat a-brown">
            <div class="stat-body">
              ...
              <div class="stat-ico ico-brown"><i class="ri-time-line"></i></div>
            </div>
          </div>
        </div>
            ...
            <div class="filterbar">
              ...
              <div class="filter" id="fltStatus">
                ...
                <div class="filter-menu">
                  ...
                  <a href="#" data-status="pending">No Evaluation</a>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4><?= $error_message ? 'Database Error' : 'No CCJE Faculty Found' ?></h4>
                        <p><?= $error_message ? 'Please check your database connection.' : 'No CCJE faculty members with teaching status found.' ?></p>
...
<div class="modal" id="evaluation-modal">
  <div class="modal-box">
    ...
    <div class="modal-foot">
      ...
      <button type="button" class="btn btn-green" id="modal-print-btn"><i class="ri-printer-line"></i> Print</button>
    </div>
  </div>
</div>
...
function filterRows() {
  ...
  rows.forEach(row => {
    ...
    const matchStatus = currentStatus === 'all' || status === currentStatus || (currentStatus === 'pending' && (!status || status === 'pending'));
...
closeBtns.forEach(btn => { btn.addEventListener('click', closeEvaluationModal); });

modal.addEventListener('click', function(e) { if (e.target === modal) closeEvaluationModal(); });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // auto-open (matches the 2026-09-03 fix already in CCJE.php/ca_eval.php).
