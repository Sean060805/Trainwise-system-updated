<?php
/* ===================================================================
   CONAH — ASSESSMENT FORM SUBMISSIONS (College of Nursing and Allied Health only)
   -------------------------------------------------------------------
   This is a CONAH-scoped copy of "Assessment Form.php". EVERY query below
   is constrained to department = 'CONAH' — a CONAH admin can only ever see
   or act on CONAH records. UI matches CONAH.php (dark rail sidebar, green
   accent, dual LSPU + CONAH logos).

   Assumed to live in the same folder as CONAH.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as CONAH.php — keeps this page inside the protected CONAH area.
...
// ----- Delete handler (POST) — only allowed for CONAH-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a CONAH user.
        $conahDept = CONAH_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $conahDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — CONAH only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . CONAH_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — CONAH only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . CONAH_DEPT . "'";
...
// ----- Count total rows (respects active filters, CONAH only) ---------
...
// ----- Main paginated query (sorted by surname, CONAH only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all CONAH rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted CONAH faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always CONAH).
$conah_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . CONAH_DEPT . "' AND status = 'accepted'");
$conah_eligible = ($conah_eligible_result && $conah_eligible_result->num_rows > 0) ? (int)$conah_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $conah_eligible > 0 ? round(($total_rows / $conah_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>CONAH Assessment Forms · LSPU TNA</title>
...
<style>
  /* Design system — same tokens as CONAH.php / admin_page.php, green-accented rail. */
  :root {
    --bg:#f5f6f8; --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(101,163,13,0.05), transparent 60%);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .scope-badge { display:inline-flex; align-items:center; gap:0.4rem; background:var(--green-soft); color:#3f6212; padding:0.3rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; margin-left:0.5rem; vertical-align:middle; }
  ...
  .stat.a-green  { --stat-accent:linear-gradient(90deg, #84cc16, #65a30d); }
  ...
  .btn-green { background:var(--green); color:#fff; box-shadow:0 8px 18px -10px rgba(101,163,13,0.55); }
  .btn-green:hover { background:#4d7c0f; }
  .btn-gold { background:var(--gold); color:#fff; box-shadow:0 8px 18px -10px rgba(202,138,4,0.55); }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--green-soft); color:#3f6212; padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .search input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(101,163,13,0.12); }
  ...
  .filter-menu a.sel { background:var(--green-soft); color:#3f6212; font-weight:600; }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/conah-logo.png" alt="CONAH" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
      ...
      <div class="rail-brand-text">
        <h1>CONAH Admin</h1>
        <p>College of Nursing & Allied Health</p>
      </div>
    </div>
    ...
    <nav class="rail-nav">
      ...
      <a href="CONAH_Assessment Form.php" class="rail-link active">
        <i class="ri-survey-line"></i><span>Assessment Form</span>
        <i class="ri-arrow-right-s-line chev"></i>
      </a>
      <a href="CONAH_Individual_Development_Plan_Form.php" class="rail-link">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip">
          ...
          <span class="avatar-meta">
            ...
            <span class="rl">CONAH Admin</span>
          </span>
        </div>
      </div>
    </header>
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>CONAH Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> CONAH only</span></h2>
            <p>Viewing submissions from College of Nursing and Allied Health faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
        ...
        <div class="grid-stats">
          <div class="stat a-green">
            <div class="stat-body">
              <div>
                <p class="stat-label">CONAH Records</p>
          ...
          <div class="stat a-gold">
            <div class="stat-body">
              ...
              <div class="stat-ico ico-gold"><i class="ri-user-star-line"></i></div>
            </div>
          </div>
          ...
          <div class="stat a-amber">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $conah_eligible ?> CONAH accepted users</p>
              </div>
        ...
        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to CONAH) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> CONAH records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Nursing and Allied Health</div>
                      </td>
                      <td>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No CONAH submissions found</h4>
                        <p>
...
<!-- HIDDEN EXPORT TABLE (CONAH rows only) -->
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Nursing and Allied Health</p></div>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF NURSING AND ALLIED HEALTH";
  doc.setFontSize(13);
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[240,253,244], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });

  doc.save("CONAH_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
