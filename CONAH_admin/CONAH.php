<?php
/* =============================================================================
   LSPU — COLLEGE OF NURSING AND ALLIED HEALTH (CONAH) DEPARTMENT ADMIN DASHBOARD
   -----------------------------------------------------------------------------
   ...
   keeping CONAH.php's original backend logic (faculty stats, evaluation
...
/* -----------------------------------------------------------------------------
   TRAINING DEMAND FORWARDED TO CONAH — HR's demand-aggregation pipeline
   ...
   Only demand HR has forwarded, with at least one CONAH requester.
   2026-09-03 - ported into this redesigned dashboard from the pre-redesign
   CONAH.php, which had this feature but this file's visual backbone (from
...
// canonicalTnaCollegeCode() rather than an exact 'CONAH' string match, so
...
// Only show demands where CONAH actually has a requester (unchanged gating) -
// but show the TRUE cross-college total, not just CONAH's own count, so the
...
/* =============================================================================
   NEW · ASSESSMENT FORM ANALYTICS (CONAH department)
...
try {
    ...
    if ($activeDeadline) {
        ...
        $eligibleRes = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = 'CONAH' AND status = 'accepted'");
        ...
        $stmt = $con->prepare("
            ...
            WHERE u.department = 'CONAH' AND u.status = 'accepted'
    ...
    error_log("Assessment Form analytics error in CONAH.php: " . $e->getMessage());
}

/* =============================================================================
   NEW · IDP (INDIVIDUAL DEVELOPMENT PLAN) FORM ANALYTICS — CONAH department
...
if ($idpTableCheck && $idpTableCheck->num_rows > 0) {
    ...
    try {
        ...
        error_log("IDP Form analytics error in CONAH.php: " . $e->getMessage());
    }
}
...
$deanNotif = getDeanNotifications($con, $user_id, 'CONAH');
...
<head>
  ...
  <title>CONAH Admin Dashboard · LSPU TNA</title>
  ...
  <link rel="stylesheet" href="../assets/css/tw-30.css">

  <!-- ===========================================================
       DESIGN SYSTEM — copied 1:1 from admin_page.php so both dashboards
       share the exact same look. CONAH-only additions are appended at the
       bottom under "CONAH EXTRAS".
       =========================================================== -->
  <style>
    :root {
      --bg:          #f5f6f8;
    ...
    *, *::before, *::after { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .ico-dark   { background: #f3f4f6; color: var(--dark); }
    ...
    .badge-green    { background: var(--green-soft); color: #3f6212; }
    .badge-gold     { background: var(--gold-soft); color: #854d0e; }
    ...
    /* ===========================================================
       CONAH EXTRAS — kept from the original CONAH.php for the faculty
    ...
    .status-submitted { background: #f0fdf4; color: #3f6212; }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="CONAH.php" class="rail-link active">
          ...
          <div class="dropdown-panel">
            ...
            <div id="notifDropdownList">
              <?php if (!empty($notifications)): ?>
                ...
                ?>
                  <div class="dropdown-item"<?= $isUnread ? ' style="background:var(--accent-soft, #f0fdf4);"' : '' ?>>
        ...
        <div class="dropdown" id="avatarDropdown">
          ...
          <button class="avatar-chip" onclick="toggleDropdown('avatarDropdown')" aria-label="Account menu">
            ...
            <span class="avatar-meta">
              ...
              <span class="rl">CONAH Dean / Admin</span>
            </span>
            ...
            <div class="dropdown-head">
              <div style="display:flex;align-items:center;gap:0.65rem;">
                ...
                <div style="line-height:1.2;">
                  ...
                  <div style="font-size:0.72rem;color:var(--muted);">College of Nursing & Allied Health</div>
                </div>
              </div>
            </div>
    ...
    <div class="content-scroll">
      <div class="content">
          ...
          <div class="stat a-gold">
            <div class="stat-body">
              <div>
                ...
                <h3 class="stat-value num" style="color:#854d0e;"><?= $stats['teaching'] ?></h3>
              </div>
              <div class="stat-ico ico-gold"><i class="fas fa-chalkboard-teacher"></i></div>
            </div>
          </div>
        ...
        <section class="grid-2col">
            ...
            <div class="card-body">
              ...
              <?php else: ?>
                <div class="empty compact">
                  ...
                  <p>Once the main admin sets an active TNA deadline, submission analytics for CONAH will appear here.</p>
                </div>
              <?php endif; ?>
            </div>
          </div>
          ...
          <div class="card hoverable">
            <div class="card-head">
              ...
              <div class="card-head-icon" style="background:var(--green-soft);color:#3f6212;"><i class="ri-pie-chart-line"></i></div>
            </div>
            <div class="card-body">
              ...
              <?php if (!empty($unevaluated_faculty)): ?>
                    ...
                    <div class="evaluation-row">
                      <div class="evaluation-info">
                        ...
                        <div class="evaluation-meta">
                          <span class="evaluation-meta-item"><i class="fas fa-building" style="color:var(--accent);"></i> College of Nursing & Allied Health</span>
                          <span class="evaluation-meta-item">
                            ...
                            <?php else: ?>
                              <i class="fas fa-user-tie" style="color:var(--dark);"></i> Non-Teaching
                            <?php endif; ?>
                          </span>
                        </div>
                      </div>
                      <div class="evaluation-actions">
                        <a href="conah_eval.php?user_id=<?= $faculty['id'] ?>" class="btn btn-green btn-sm">Evaluate Now</a>
                      </div>
                    </div>
                  <?php endforeach; ?>
                </div>
                ...
                <?php if (count($unevaluated_faculty) > 5): ?>
                  <div style="margin-top:1rem;text-align:center;">
                    <a href="conah_eval.php" class="btn btn-ghost btn-sm">
        ...
        <!-- ===========================================================
             ...
             Report to HR modal now live in CONAH_Training_Demand.php.
             =========================================================== -->
        <a href="CONAH_Training_Demand.php" style="display:block;margin-top:1.15rem;">
    ...
    <div class="bg-white rounded-2xl shadow-2xl modal-container w-full max-w-6xl">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="mt-8 flex justify-end space-x-4 pt-6 border-t" style="border-color:var(--line);">
                ...
                <?php endif; ?>
                <a href="conah_eval.php?evaluation_id=<?= $evaluation_details['id'] ?>&user_id=<?= $evaluation_details['user_id'] ?>" class="btn btn-primary"><i class="ri-edit-line"></i>Edit Evaluation</a>
            </div>
        </div>
    </div>
</div>
...
function updateNotifications() {
  ...
  // which 404s from inside CONAH_admin/ since the endpoint lives at the
    ...
    new Chart(progressEl.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#059669', '#65a30d', '#ca8a04'],
    ...
    new Chart(distEl.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['rgba(202,138,4,0.85)', 'rgba(31,41,55,0.85)'],
          borderColor: ['#ca8a04', '#1f2937'],
