<?php
/* ===================================================================
   ASSESSMENT FORM SUBMISSIONS — Admin view
   -------------------------------------------------------------------
   UI redesigned to match admin_page.php (enterprise dashboard system:
   dark slate rail, indigo accents, off-white canvas, JetBrains Mono
   for data). ALL backend logic below is preserved from the original
   page — same query params, filters, delete handler, pagination,
   detail lookup, and export query.
   =================================================================== */
...
// ----- LSPU departments (code → full name) -------------------------
$departments = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    ...
    if ($delete_id > 0) {
        ...
        if (!empty($department_filter))   { $redirect_url .= "&department=" . urlencode($department_filter); }
        header("Location: $redirect_url");
        exit();
    }
}

// ----- Distinct years / months / statuses (for filter menus) -------
...
// ----- Detail lookup for the View modal (server-side fallback) -----
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
...
// ----- Count total rows (respects active filters) ------------------
...
if (!empty($department_filter)) { $count_sql .= " AND u.department = '$department_filter'"; }
...
function getSurname($name) {
    $parts = explode(' ', $name);
    return end($parts); // last token = surname
}

// ----- Main paginated query (sorted by surname) --------------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
if (!empty($search))            { $sql .= " AND (u.name LIKE '%$search%' OR u.department LIKE '%$search%' OR u.email LIKE '%$search%')"; }
if (!empty($teaching_status))   { $sql .= " AND u.teaching_status = '$teaching_status'"; }
if (!empty($department_filter)) { $sql .= " AND u.department = '$department_filter'"; }
...
// ----- Export query (all rows, no pagination, same sort) -----------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
if (!empty($search))            { $export_sql .= " AND (u.name LIKE '%$search%' OR u.department LIKE '%$search%' OR u.email LIKE '%$search%')"; }
if (!empty($teaching_status))   { $export_sql .= " AND u.teaching_status = '$teaching_status'"; }
if (!empty($department_filter)) { $export_sql .= " AND u.department = '$department_filter'"; }
...
/* ----- Derived stat counts (computed from data already fetched;
         no extra DB queries). Reflects the current filter view. ----- */
$stat_teaching = 0;
$stat_nonteaching = 0;
$dept_seen = [];
foreach ($all_rows_for_export as $r) {
    ...
    if (!empty($r['department'])) { $dept_seen[$r['department']] = true; }
}
$stat_departments = count($dept_seen);

// Human-readable label for the active period (used in the header + stat).
...
<head>
  ...
  <title>Assessment Forms · LSPU TNA</title>
...
<style>
  /* ==========================================================
     DESIGN SYSTEM — shared with admin_page.php
     Sections: 1 tokens · 2 base · 3 shell · 4 sidebar ·
     5 topbar/dropdowns · 6 page header · 7 cards/stats ·
     8 buttons · 9 badges · 10 tables · 11 forms ·
     12 filter bar · 13 modal · 14 profile cards (modal content) ·
     15 toasts · 16 empty · 17 pagination · 18 scrollbar ·
     19 responsive · 20 motion
     ========================================================== */
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail {
    ...
    position:sticky; top:0; height:100vh; border-right:1px solid rgba(0,0,0,0.2); z-index: 50;
  }
  ...
  .card-head { display:flex; align-items:center; justify-content:space-between; padding:1.05rem 1.3rem; border-bottom:1px solid var(--line-soft); }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--accent-soft); color:var(--accent-ink); padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  /* 11. FORMS / SEARCH INPUT */
  .input { width:100%; font-family:'Inter',sans-serif; font-size:0.875rem; color:var(--ink); padding:0.7rem 0.9rem; border:1px solid var(--line); border-radius:var(--radius-sm); background:var(--surface); transition:border-color 0.18s var(--ease), box-shadow 0.18s var(--ease); }
  .input::placeholder { color:var(--faint); }
  .input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(79,70,229,0.12); }
  ...
  /* 12. FILTER BAR (click dropdowns wrapping the original href links) */
  ...
  .filter-menu { position:absolute; left:0; top:calc(100% + 0.4rem); min-width:230px; max-height:320px; overflow-y:auto; background:var(--surface); border:1px solid var(--line); border-radius:14px; box-shadow:var(--shadow-pop); padding:0.4rem; z-index:50; opacity:0; transform:translateY(-6px); pointer-events:none; transition:opacity 0.16s var(--ease), transform 0.16s var(--ease); }
  .filter.open .filter-menu { opacity:1; transform:translateY(0); pointer-events:auto; }
  .filter-menu a { display:flex; align-items:center; justify-content:space-between; gap:0.5rem; padding:0.6rem 0.75rem; border-radius:9px; font-size:0.85rem; color:var(--ink-2); transition:background 0.14s var(--ease); }
  .filter-menu a:hover { background:var(--surface-2); }
  .filter-menu a.sel { background:var(--accent-soft); color:var(--accent-ink); font-weight:600; }
  ...
  /* 14. PROFILE CARDS — used by JS-injected modal content */
  ...
  .profile-card h4 { color:var(--ink); }
  .profile-card h5, .profile-card h6 { font-family:'Inter',sans-serif; }
  ...
  .pg { display:inline-flex; align-items:center; justify-content:center; min-width:38px; height:38px; padding:0 0.5rem; border:1px solid var(--line); border-radius:10px; background:var(--surface); color:var(--ink-2); font-size:0.85rem; font-weight:600; transition:background 0.16s var(--ease), border-color 0.16s var(--ease), color 0.16s var(--ease); }
...
<!-- =============================================================
     ...
     State on #app: .rail-collapsed (desktop mini) · .rail-open (mobile drawer)
     ============================================================= -->
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="Assessment Form.php" class="rail-link active">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip" aria-label="Account">
          ...
          <span class="avatar-meta">
            ...
            <span class="rl">Assessment Forms</span>
          </span>
        </div>
      </div>
    </header>
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>Assessment Form Submissions</h2>
            <p>View and manage all submitted assessment forms for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
        ...
        <!-- STAT CARDS (derived from the current filtered result set) -->
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              ...
              <div class="stat-ico ico-indigo"><i class="ri-file-list-3-line"></i></div>
            </div>
          </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              ...
              <div class="stat-ico ico-sky"><i class="ri-user-line"></i></div>
            </div>
          </div>
          <div class="stat a-amber">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?= $stat_departments ?></p>
                <p class="stat-sub">represented</p>
              </div>
              <div class="stat-ico ico-amber"><i class="ri-building-line"></i></div>
            </div>
          </div>
        </div>

        <!-- FILTER BAR — click dropdowns wrapping the original href filters -->
        <!-- overflow:visible so open dropdown menus aren't clipped by the card edge -->
            ...
            <div class="filterbar">
              ...
              <div class="filter" id="fltDept">
                <button class="filter-btn" type="button" onclick="toggleFilter('fltDept')">
                  <span class="flt-label">Dept</span>
                  <span class="flt-value <?= !empty($department_filter) ? 'is-set' : '' ?>"><?= empty($department_filter) ? 'All' : htmlspecialchars($department_filter) ?></span>
                  <i class="ri-arrow-down-s-line caret"></i>
                </button>
                <div class="filter-menu">
                  <a href="?year=<?= $selected_year ?>&month=<?= $selected_month ?>&teaching_status=<?= urlencode($teaching_status) ?>&search=<?= urlencode($search) ?>"
                     class="<?= empty($department_filter) ? 'sel' : '' ?>">All Departments</a>
                  <?php foreach ($departments as $abbr => $name): ?>
                    <a href="?year=<?= $selected_year ?>&month=<?= $selected_month ?>&teaching_status=<?= urlencode($teaching_status) ?>&department=<?= urlencode($abbr) ?>&search=<?= urlencode($search) ?>"
                       class="<?= $department_filter == $abbr ? 'sel' : '' ?>"><?= htmlspecialchars($abbr . ' — ' . $name) ?></a>
                  <?php endforeach; ?>
                </div>
              </div>

              <!-- Search (submits via JS, same param as original) -->
              <div class="search">
                <i class="ri-search-line"></i>
                <input type="search" id="search-input" placeholder="Search name, department, or email…"
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> total records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      <td>
                        ...
                        <div class="cell-sub num"><?= date('M d, Y', strtotime($row['submission_date'])) ?></div>
                      </td>
                      <td>
                        <span class="dept-badge"><?= htmlspecialchars($row['department']) ?></span>
                        <div class="cell-sub"><?= htmlspecialchars($departments[$row['department']] ?? $row['department']) ?></div>
                      </td>
                      <td>
                    ...
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <p>
                          ...
                          <?php if (!empty($search) || !empty($teaching_status) || !empty($department_filter)): ?>with current filters<?php endif; ?>
                        </p>
                      </div>
                    </td>
                  </tr>
        ...
        <div class="pager">
          ...
          <nav class="pager-nav" aria-label="Pagination">
            <?php
              // Reusable query-string suffix for page links (keeps active filters).
              $qs = function($p) use ($selected_year, $selected_month, $search, $teaching_status, $department_filter) {
...
<!-- =============================================================
     VIEW MODAL — employee profile (content injected by JS fetch)
     ============================================================= -->
<div class="modal" id="view-modal">
  <div class="modal-box">
    ...
    <div class="modal-body">
      <div id="modal-content"><!-- JS loads content here --></div>
    </div>
...
<!-- =============================================================
     HIDDEN EXPORT TABLE — source for the PDF (generatePDF reads this)
     ============================================================= -->
...
<!-- PDF libraries (unchanged) -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.5.28/jspdf.plugin.autotable.min.js"></script>

<script>
/* ============================================================
   A. TOAST SYSTEM (shared with admin_page.php)
   ============================================================ */
...
/* ============================================================
   B. SIDEBAR — collapse (desktop) + drawer (mobile)
   ============================================================ */
...
/* ============================================================
   C. FILTER DROPDOWNS (click to open; href links do the filtering)
   ============================================================ */
...
// Close any open filter when clicking outside of it.
...
// Escape closes the view modal + any open filter.
...
/* ============================================================
   E. SCROLL POSITION (preserve across pagination navigation)
   ============================================================ */
...
/* ============================================================
   F. SEARCH (same query param + reset to page 1 as original)
   ============================================================ */
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Pagination links preserve scroll position.
  ...
  // ----- View details (fetch + inject into modal) -----
          ...
          if (data.success) {
            ...
            modalContent.innerHTML = `
                ...
                <div class="flex flex-col md:flex-row gap-6">
                  <div class="md:w-1/3">
                    <div class="flex items-center space-x-4 mb-6">
                      ...
                      <div>
                        <h4 class="text-xl font-bold" style="color:var(--ink);">${assessment.name || 'N/A'}</h4>
                    ...
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Educational Attainment</h5><p style="color:var(--ink-2);">${assessment.educationalAttainment || 'Not specified'}</p></div>
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Specialization</h5><p style="color:var(--ink-2);">${assessment.specialization || 'Not specified'}</p></div>
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Designation</h5><p style="color:var(--ink-2);">${assessment.designation || 'Not specified'}</p></div>
                    </div>
                  </div>
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">${assessment.department || 'N/A'}</p></div>
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Years in LSPU</h5><p style="color:var(--ink-2);">${assessment.yearsInLSPU || 'Not specified'}</p></div>
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Type of Employment</h5><p style="color:var(--ink-2);">${assessment.teaching_status || 'Not specified'}</p></div>
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Email</h5><p style="color:var(--ink-2);word-break:break-all;">${assessment.email || 'Not specified'}</p></div>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    ${assessment.training_history.map(training => `
                      <div class="training-item">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                          <div><h6 class="text-sm font-medium" style="color:var(--muted);">Training/Seminar</h6><p style="color:var(--ink);font-weight:600;">${training.training || 'N/A'}</p></div>
                          ...
                          <div><h6 class="text-sm font-medium" style="color:var(--muted);">Venue</h6><p style="color:var(--ink-2);">${training.venue || 'N/A'}</p></div>
                        </div>
                      </div>
                    `).join('')}
                  </div>
              ...
              <div class="profile-card" style="margin-bottom:1.25rem;">
                ...
                <div style="background:var(--surface-2);border:1px solid var(--line);padding:1rem;border-radius:12px;">
                  <p style="color:var(--ink-2);white-space:pre-line;">${assessment.desired_skills || 'Not specified'}</p>
                </div>
              </div>
              ...
              <div class="profile-card" style="margin-bottom:1.25rem;">
                ...
                <div style="background:var(--surface-2);border:1px solid var(--line);padding:1rem;border-radius:12px;">
                  <p style="color:var(--ink-2);white-space:pre-line;">${assessment.comments || 'Not specified'}</p>
                </div>
              </div>

              <div class="profile-card" id="dean-recs-card">
                <h4 class="text-lg font-bold" style="color:var(--ink);margin-bottom:0.35rem;">
                  <i class="ri-robot-2-line" style="color:var(--accent);"></i> AI-Recommended Trainings
                </h4>
                <p style="color:var(--muted);font-size:0.85rem;margin-bottom:1rem;">
                  What the AI recommended for this employee and where each stands. Accepted trainings pool with the rest of the college under <strong>Training Demand</strong> on the main dashboard.
                </p>
                ${assessment.ml_recommendations && assessment.ml_recommendations.length > 0 ? `
                  <div class="space-y-3">
                    ${assessment.ml_recommendations.map(rec => `
                      <div style="background:var(--surface-2);border:1px solid var(--line);padding:0.9rem 1rem;border-radius:12px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;">
                        <div>
                          <p style="color:var(--ink);font-weight:600;">${rec.title}</p>
                          <p style="color:var(--muted);font-size:0.8rem;margin-top:2px;">${rec.training_type || 'Training'}</p>
                        </div>
                        <span class="badge badge-info">${rec.status}</span>
                      </div>
                    `).join('')}
                  </div>
                ` : `<p style="color:var(--muted);font-style:italic;">No AI recommendations generated yet for this employee.</p>`}
              </div>`;

            // Wire up Print (posts a hidden form to the PDF generator).
            ...
            modalContent.innerHTML = `
              <div class="empty"><i class="ri-error-warning-line" style="color:var(--bad);"></i>
                <h4>Error loading assessment details</h4><p>Please try again later.</p></div>`;
          }
        })
        .catch(error => {
          ...
          modalContent.innerHTML = `
            <div class="empty"><i class="ri-error-warning-line" style="color:var(--bad);"></i>
              <h4>Error loading assessment details</h4><p>Please try again later.</p></div>`;
          showToast('err', 'Load failed', 'Could not load assessment details.');
        });
    });
  });
...
/* ============================================================
   H. PDF EXPORT (reads #full-data-table — logic unchanged)
   ============================================================ */
function generatePDF() {
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[230,230,230], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });
