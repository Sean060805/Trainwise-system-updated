<?php
/* ===================================================================
   CTE IDP SUBMISSIONS — Department view for College of Teacher Education
   =================================================================== */
...
$forwardedDemandCount = getForwardedDemandCountForCollege($con, 'CTE');
...
function processSignatureUpload($file, $role, $formId)
{
    ...
    $uploadDir = '../uploads/signatures/cte/';
...
/*
|--------------------------------------------------------------------------
| Main employee list - FILTERED TO CTE DEPARTMENT
|--------------------------------------------------------------------------
*/
$query = "
    SELECT 
    ...
    WHERE f.status = 'submitted' AND u.department = 'CTE'
...
$statsResult = $con->query("
    ...
    FROM users 
    WHERE department = 'CTE' AND role = 'user' 
    AND teaching_status IS NOT NULL AND teaching_status != ''
");
if ($statsResult) {
    $cteStats['total_employees'] = $statsResult->fetch_assoc()['count'];
}
$cteStats['with_idp'] = count($employees);
...
$hrResult = $con->query("
    ...
    FROM idp_forms f
    JOIN users u ON f.user_id = u.id
    WHERE f.status = 'submitted_to_hr' AND u.department = 'CTE'
");
if ($hrResult) {
    $cteStats['submitted_to_hr'] = $hrResult->fetch_assoc()['count'];
}
...
<head>
  ...
  <title>CTE IDP Submissions · LSPU TNA</title>
  ...
  <!-- TAILWIND -->
  <link rel="stylesheet" href="../assets/css/tw-14.css">
<style>
  /* ==========================================================
     DESIGN SYSTEM — CTE Blue/Yellow Accent Version
     ========================================================== */
  ...
  :root {
    ...
    --accent:#3b82f6; --accent-700:#2563eb; --accent-soft:#eff6ff; --accent-ink:#1e40af;
    --cteblue:#3b82f6; --cteblue-light:#60a5fa; --cteblue-soft:#eff6ff;
    --cteyellow:#fbbf24; --cteyellow-light:#fcd34d; --cteyellow-soft:#fffbeb;
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:38px; height:38px; border-radius:10px; flex-shrink:0; background:linear-gradient(135deg, var(--cteblue), var(--cteblue-light)); display:flex; align-items:center; justify-content:center; }
  ...
  .avatar { width:34px; height:34px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--cteblue), var(--cteblue-light)); color:#fff; font-weight:700; font-size:0.85rem; display:flex; align-items:center; justify-content:center; font-family:'Plus Jakarta Sans',sans-serif; }
  ...
  .date-chip i { color:var(--cteblue); font-size:1.05rem; }
  ...
  .stat::after { content:''; position:absolute; inset:0 0 auto 0; height:3px; background:var(--stat-accent, var(--cteblue)); }
  .stat.a-cteblue  { --stat-accent:linear-gradient(90deg, var(--cteblue-light), var(--cteblue)); }
  ...
  .btn-cteblue { background:var(--cteblue); color:#fff; box-shadow:0 8px 18px -10px rgba(59,130,246,0.6); }
  ...
  .view-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--cteblue); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.85rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(59,130,246,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
  ...
  .department-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--cteblue-soft); color:var(--cteblue); padding:0.32rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  table.data tbody tr:hover { background:var(--cteblue-soft); }
  table.data tbody tr:last-child td { border-bottom:none; }
  .cell-name { font-weight:600; color:var(--ink); }
  .emp-avatar { width:38px; height:38px; border-radius:10px; background:var(--cteblue-soft); color:var(--cteblue); display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .search input:focus { outline:none; border-color:var(--cteblue); box-shadow:0 0 0 4px rgba(59,130,246,0.12); }
  ...
  .search-go { position:absolute; right:6px; top:50%; transform:translateY(-50%); width:32px; height:32px; border:none; border-radius:8px; background:var(--cteblue); color:#fff; cursor:pointer; display:inline-flex; align-items:center; justify-content:center; transition:background 0.18s var(--ease); }
  ...
  .filter.open .filter-btn { border-color:var(--cteblue); }
  ...
  .status-select:focus { outline:none; border-color:var(--cteblue); box-shadow:0 0 0 4px rgba(59,130,246,0.12); }
  ...
  .modal-table tr:hover td { background:var(--cteblue-soft); }
  .modal-table tr:last-child td { border-bottom:none; }
  .checkbox-custom { width:18px; height:18px; accent-color:var(--cteblue); }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="CTE.php" class="rail-link">
      ...
      <a href="CTE_Individual_Development_Plan_Form.php" class="rail-link active">
  ...
  <div class="app-main">
    <header class="topbar">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip" aria-label="Account">
          ...
          <span class="avatar-meta"><span class="nm"><?= htmlspecialchars($adminName) ?></span> <span class="rl">CTE Admin</span></span>
        </div>
      </div>
    </header>
    ...
    <div class="content-scroll">
      <div class="content">
        <div class="page-head">
          <div>
            <p class="eyebrow">Individual Development Plans · CTE</p>
            <h2>CTE IDP Submissions</h2>
            <p>View and manage IDP forms submitted by College of Teacher Education faculty and staff.</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">Total CTE Faculty</p>
                <p class="stat-value num"><?php echo $cteStats['total_employees']; ?></p>
                <p class="stat-sub">eligible for IDP</p>
              </div>
              <div class="stat-ico ico-indigo"><i class="ri-team-line"></i></div>
            </div>
          </div>
          <div class="stat a-cteblue">
            <div class="stat-body">
              <div>
                <p class="stat-label">With IDP Submitted</p>
                <p class="stat-value num"><?php echo $cteStats['with_idp']; ?></p>
                <p class="stat-sub"><?php echo $cteStats['total_employees'] > 0 ? round(($cteStats['with_idp'] / $cteStats['total_employees']) * 100, 1) : 0; ?>% of faculty</p>
              </div>
              <div class="stat-ico ico-cteblue"><i class="ri-file-list-3-line"></i></div>
            </div>
          </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $cteStats['submitted_to_hr']; ?></p>
        ...
        <div class="card" style="margin-bottom:1.4rem; overflow:visible; position:relative; z-index:20;">
          <div class="card-body" style="padding:1.05rem 1.15rem;">
            <div class="filterbar">
              <div style="display:inline-flex;align-items:center;gap:0.55rem;height:42px;padding:0 0.9rem;border:1px solid var(--cteblue);border-radius:var(--radius-sm);background:var(--cteblue-soft);">
                <i class="ri-building-line" style="color:var(--cteblue);"></i>
                <span style="font-weight:600;color:var(--cteblue);font-size:0.85rem;">CTE</span>
                <span style="font-size:0.7rem;color:var(--cteblue);opacity:0.7;">College of Teacher Education</span>
              </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p style="font-size:0.82rem; color:var(--muted);">
            ...
            of <span class="num"><?= count($employees) ?></span> CTE employees with submitted IDPs
          </p>
        </div>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No IDP submissions found for CTE</h4>
                        <p>CTE faculty and staff will appear here once they submit their IDP forms.</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($employees as $employee): ?>
                    ...
                    <tr class="employee-item" data-department="<?= htmlspecialchars($employee['department']) ?>">
                      ...
                      <td><span class="num" style="font-weight:700; color:var(--cteblue); font-size:1.05rem;"><?= (int) $employee['total_idps'] ?></span></td>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="signature-upload-card">
                    ...
                    <input
                      ...
                      class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-cteblue file:text-white hover:file:bg-[#2563eb] cursor-pointer signature-file-input"
  ...
  function showIDPModal(userId, userName, expandFormId = null) {
    ...
    modalContent.innerHTML = `
      <div class="flex justify-center items-center" style="padding:3rem 0;">
        <div style="width:36px;height:36px;border:3px solid var(--cteblue-soft);border-top-color:var(--cteblue);border-radius:50%;animation:spin 0.8s linear infinite;"></div>
      </div>
