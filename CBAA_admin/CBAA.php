<?php
/* =============================================================================
   LSPU — COLLEGE OF BUSINESS, ADMINISTRATION AND ACCOUNTANCY (CBAA) 
   ...
   keeping CBAA.php's original backend logic (faculty stats, evaluation
...
/* -----------------------------------------------------------------------------
   TRAINING DEMAND FORWARDED TO CBAA — HR's demand-aggregation pipeline
   ...
   Only demand HR has forwarded, with at least one CBAA requester.
   2026-09-03 - ported into this redesigned dashboard from the pre-redesign
   CBAA.php, which had this feature but this file's visual backbone (from
...
// canonicalTnaCollegeCode() rather than an exact 'CBAA' string match, so
...
// Only show demands where CBAA actually has a requester (unchanged gating) -
// but show the TRUE cross-college total, not just CBAA's own count, so the
...
try {
    ...
    // CBAA's own (same class of bug as evaluated_this_year below). See
    // CLAUDE.md.
    $evaluation_stats_result = $con->query("
        ...
        FROM evaluations
        WHERE EXISTS (
            SELECT 1 FROM users u WHERE u.id = evaluations.user_id AND u.department = 'CBAA'
        )
    ");
    ...
    // 2026-09-04 fix - was counting every college's evaluations, not just
    // CBAA's own (inflated evaluated_this_year and could push progress
...
/* =============================================================================
   NEW · ASSESSMENT FORM ANALYTICS (CBAA department)
...
try {
    ...
    if ($activeDeadline) {
        ...
        $eligibleRes = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = 'CBAA' AND status = 'accepted'");
        ...
        $stmt = $con->prepare("
            ...
            WHERE u.department = 'CBAA' AND u.status = 'accepted'
    ...
    error_log("Assessment Form analytics error in CBAA.php: " . $e->getMessage());
}

/* =============================================================================
   NEW · IDP (INDIVIDUAL DEVELOPMENT PLAN) FORM ANALYTICS — CBAA department
...
if ($idpTableCheck && $idpTableCheck->num_rows > 0) {
    ...
    try {
        ...
        error_log("IDP Form analytics error in CBAA.php: " . $e->getMessage());
    }
}
...
$deanNotif = getDeanNotifications($con, $user_id, 'CBAA');
...
<head>
  ...
  <title>CBAA Admin Dashboard · LSPU TNA</title>
  ...
  <link rel="stylesheet" href="../assets/css/tw-39.css">

  <!-- ===========================================================
       DESIGN SYSTEM — copied 1:1 from admin_page.php so both dashboards
       share the exact same look. CBAA-only additions are appended at the
       bottom under "CBAA EXTRAS".
       =========================================================== -->
  <style>
    :root {
      --bg:          #f5f6f8;
    ...
    *, *::before, *::after { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .stat.a-gold    { --stat-accent: linear-gradient(90deg, #fbbf24, #f59e0b); }
    ...
    .btn-gold { background: var(--gold); color: #fff; box-shadow: 0 8px 18px -10px rgba(245,158,11,0.55); }
    ...
    /* ===========================================================
       CBAA EXTRAS — kept from the original CBAA.php for the faculty
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      ...
      <div class="rail-brand-text">
        <h1>CBAA Admin</h1>
        <p>College of Business, Admin & Accountancy</p>
      </div>
    </div>

    <nav class="rail-nav">
      <p class="rail-section-label">Overview</p>
      <a href="CBAA.php" class="rail-link active">
      ...
      <a href="cbaa_eval.php" class="rail-link">
          ...
          <div class="dropdown-panel">
            ...
            <div id="notifDropdownList">
              <?php if (!empty($notifications)): ?>
                ...
                ?>
                  <div class="dropdown-item"<?= $isUnread ? ' style="background:var(--accent-soft, #fef3c7);"' : '' ?>>
        ...
        <div class="dropdown" id="avatarDropdown">
          ...
          <button class="avatar-chip" onclick="toggleDropdown('avatarDropdown')" aria-label="Account menu">
            ...
            <span class="avatar-meta">
              ...
              <span class="rl">CBAA Dean / Admin</span>
            </span>
            ...
            <div class="dropdown-head">
              <div style="display:flex;align-items:center;gap:0.65rem;">
                ...
                <div style="line-height:1.2;">
                  ...
                  <div style="font-size:0.72rem;color:var(--muted);">College of Business, Admin & Accountancy</div>
                </div>
              </div>
            </div>
    ...
    <div class="content-scroll">
      <div class="content">
        ...
        <section class="grid-stats">
          <div class="stat a-gold">
            <div class="stat-body">
              ...
              <div class="stat-ico ico-gold"><i class="fas fa-users"></i></div>
            </div>
          </div>
            ...
            <div class="card-body">
              ...
              <?php else: ?>
                <div class="empty compact">
                  ...
                  <p>Once the main admin sets an active TNA deadline, submission analytics for CBAA will appear here.</p>
                </div>
              <?php endif; ?>
            </div>
          </div>
              ...
              <?php if (!empty($unevaluated_employees)): ?>
                  ...
                  <?php foreach (array_slice($unevaluated_employees, 0, 5) as $employee): ?>
                    <div class="evaluation-row">
                      ...
                      <div class="evaluation-actions">
                        <a href="cbaa_eval.php?user_id=<?= $employee['id'] ?>" class="btn btn-gold btn-sm">Evaluate Now</a>
                      </div>
                    </div>
                  <?php endforeach; ?>
                </div>
                ...
                <?php if (count($unevaluated_employees) > 5): ?>
                  <div style="margin-top:1rem;text-align:center;">
                    <a href="cbaa_eval.php" class="btn btn-ghost btn-sm">
        ...
        <!-- ===========================================================
             ...
             Report to HR modal now live in CBAA_Training_Demand.php.
             =========================================================== -->
        <a href="CBAA_Training_Demand.php" style="display:block;margin-top:1.15rem;">
    ...
    <div class="bg-white rounded-2xl shadow-2xl modal-container w-full max-w-6xl">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="mt-8 flex justify-end space-x-4 pt-6 border-t" style="border-color:var(--line);">
                ...
                <?php endif; ?>
                <a href="cbaa_eval.php?evaluation_id=<?= $evaluation_details['id'] ?>&user_id=<?= $evaluation_details['user_id'] ?>" class="btn btn-primary"><i class="ri-edit-line"></i>Edit Evaluation</a>
            </div>
        </div>
    </div>
</div>
...
function updateNotifications() {
  ...
  // which 404s from inside CBAA_admin/ since the endpoint lives at the
    ...
    .then(data => {
      ...
      if (items.length > 0) {
        ...
        items.forEach(notification => {
          ...
          if (isUnread) item.style.background = '#fef3c7';
          item.innerHTML = `
    ...
    new Chart(progressEl.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#059669', '#f59e0b', '#d97706'],
    ...
    new Chart(distEl.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['rgba(5,150,105,0.85)', 'rgba(245,158,11,0.85)'],
