<?php
/* ===================================================================
   CBAA — ASSESSMENT FORM SUBMISSIONS (College of Business, Administration and Accountancy only)
   -------------------------------------------------------------------
   This is a CBAA-scoped copy of "Assessment Form.php". EVERY query below
   is constrained to department = 'CBAA' — a CBAA admin can only ever see
   or act on CBAA records. UI matches CBAA.php (dark rail sidebar, gold
   accent, dual LSPU + CBAA logos).

   Assumed to live in the same folder as CBAA.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as CBAA.php — keeps this page inside the protected CBAA area.
...
// ----- Delete handler (POST) — only allowed for CBAA-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a CBAA user.
        $cbaaDept = CBAA_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $cbaaDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — CBAA only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . CBAA_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — CBAA only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . CBAA_DEPT . "'";
...
// ----- Count total rows (respects active filters, CBAA only) ---------
...
// ----- Main paginated query (sorted by surname, CBAA only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all CBAA rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted CBAA faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always CBAA).
$cbaa_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . CBAA_DEPT . "' AND status = 'accepted'");
$cbaa_eligible = ($cbaa_eligible_result && $cbaa_eligible_result->num_rows > 0) ? (int)$cbaa_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $cbaa_eligible > 0 ? round(($total_rows / $cbaa_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>CBAA Assessment Forms · LSPU TNA</title>
...
<style>
  /* Design system — same tokens as CBAA.php / admin_page.php, gold-accented rail. */
  :root {
    --bg:#f5f6f8; --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(245,158,11,0.05), transparent 60%);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:32px; height:32px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--gold), var(--gold-light)); display:flex; align-items:center; justify-content:center; }
  ...
  .ico-gold  { background:var(--accent-soft); color:var(--accent); }
  ...
  .search input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(217,119,6,0.12); }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/cbaa-logo.png" alt="CBAA" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="CBAA_Assessment Form.php" class="rail-link active">
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>CBAA Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> CBAA only</span></h2>
            <p>Viewing submissions from College of Business, Administration and Accountancy faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
        ...
        <div class="grid-stats">
          <div class="stat a-gold">
            <div class="stat-body">
              <div>
                <p class="stat-label">CBAA Records</p>
          ...
          <div class="stat a-amber">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $cbaa_eligible ?> CBAA accepted users</p>
              </div>
        ...
        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to CBAA) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> CBAA records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Business, Administration and Accountancy</div>
                      </td>
                      <td>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No CBAA submissions found</h4>
                        <p>
...
<!-- HIDDEN EXPORT TABLE (CBAA rows only) -->
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Business, Administration and Accountancy</p></div>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF BUSINESS, ADMINISTRATION AND ACCOUNTANCY";
  doc.setFontSize(13);
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[254,243,199], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });

  doc.save("CBAA_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
