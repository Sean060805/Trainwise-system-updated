<?php
/* ===================================================================
   CBAA IDP SUBMISSIONS — Department view for College of Business, Administration and Accountancy
   =================================================================== */
...
function processSignatureUpload($file, $role, $formId)
{
    ...
    $uploadDir = '../uploads/signatures/cbaa/';
...
/*
|--------------------------------------------------------------------------
| Main employee list - FILTERED TO CBAA DEPARTMENT
|--------------------------------------------------------------------------
*/
$query = "
    SELECT 
    ...
    WHERE f.status = 'submitted' AND u.department = 'CBAA'
...
$statsResult = $con->query("
    ...
    FROM users 
    WHERE department = 'CBAA' AND role = 'user' 
    AND teaching_status IS NOT NULL AND teaching_status != ''
");
if ($statsResult) {
    $cbaaStats['total_employees'] = $statsResult->fetch_assoc()['count'];
}
$cbaaStats['with_idp'] = count($employees);
...
$hrResult = $con->query("
    ...
    FROM idp_forms f
    JOIN users u ON f.user_id = u.id
    WHERE f.status = 'submitted_to_hr' AND u.department = 'CBAA'
");
if ($hrResult) {
    $cbaaStats['submitted_to_hr'] = $hrResult->fetch_assoc()['count'];
}
...
<head>
  ...
  <title>CBAA IDP Submissions · LSPU TNA</title>
...
<style>
  /* ==========================================================
     DESIGN SYSTEM — CBAA Gold Accent Version
     ========================================================== */
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .view-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--gold); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.85rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(245,158,11,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
  .view-btn:hover { background:#d97706; }
  ...
  .department-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--gold-soft); color:#92400e; padding:0.32rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .status-select:focus { outline:none; border-color:var(--gold); box-shadow:0 0 0 4px rgba(245,158,11,0.12); }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="CBAA_Individual_Development_Plan_Form.php" class="rail-link active">
  ...
  <div class="app-main">
    ...
    <div class="content-scroll">
      <div class="content">
        <div class="page-head">
          <div>
            <p class="eyebrow">Individual Development Plans · CBAA</p>
            <h2>CBAA IDP Submissions</h2>
            <p>View and manage IDP forms submitted by College of Business, Administration and Accountancy faculty and staff.</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">Total CBAA Faculty</p>
                <p class="stat-value num"><?php echo $cbaaStats['total_employees']; ?></p>
          ...
          <div class="stat a-gold">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $cbaaStats['with_idp']; ?></p>
                <p class="stat-sub"><?php echo $cbaaStats['total_employees'] > 0 ? round(($cbaaStats['with_idp'] / $cbaaStats['total_employees']) * 100, 1) : 0; ?>% of faculty</p>
              </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $cbaaStats['submitted_to_hr']; ?></p>
          ...
          <div class="card-body" style="padding:1.05rem 1.15rem;">
            <div class="filterbar">
              <div style="display:inline-flex;align-items:center;gap:0.55rem;height:42px;padding:0 0.9rem;border:1px solid var(--gold);border-radius:var(--radius-sm);background:var(--gold-soft);">
                <i class="ri-building-line" style="color:#92400e;"></i>
                <span style="font-weight:600;color:#92400e;font-size:0.85rem;">CBAA</span>
                <span style="font-size:0.7rem;color:#92400e;opacity:0.7;">College of Business, Admin & Accountancy</span>
              </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p style="font-size:0.82rem; color:var(--muted);">
            ...
            of <span class="num"><?= count($employees) ?></span> CBAA employees with submitted IDPs
          </p>
        </div>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No IDP submissions found for CBAA</h4>
                        <p>CBAA faculty and staff will appear here once they submit their IDP forms.</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="signature-upload-card">
                    ...
                    <input
                      ...
                      class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-gold file:text-white hover:file:bg-[#d97706] cursor-pointer signature-file-input"
