<?php
/* ===================================================================
   CCS — ASSESSMENT FORM SUBMISSIONS (College of Computer Studies only)
   -------------------------------------------------------------------
   This is a CCS-scoped copy of "Assessment Form.php". EVERY query below
   is constrained to department = 'CCS' — a CCS admin can only ever see
   or act on CCS records. UI matches CCS.php (dark rail sidebar, gold
   accent, dual LSPU + CCS logos).

   Assumed to live in the same folder as CCS.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as CCS.php — keeps this page inside the protected CCS area.
...
// 2026-09-04 - sidebar "Training Demand" badge count, same helper CCS.php
// and CCS_Training_Demand.php use, so the number matches everywhere.
require_once '../ml_recommendations.php';
$forwardedDemandCount = getForwardedDemandCountForCollege($con, 'CCS');
...
// ----- Delete handler (POST) — only allowed for CCS-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a CCS user.
        $ccsDept = CCS_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $ccsDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — CCS only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . CCS_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — CCS only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . CCS_DEPT . "'";
...
// ----- Count total rows (respects active filters, CCS only) ---------
...
// ----- Main paginated query (sorted by surname, CCS only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all CCS rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted CCS faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always CCS).
$ccs_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . CCS_DEPT . "' AND status = 'accepted'");
$ccs_eligible = ($ccs_eligible_result && $ccs_eligible_result->num_rows > 0) ? (int)$ccs_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $ccs_eligible > 0 ? round(($total_rows / $ccs_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>CCS Assessment Forms · LSPU TNA</title>
  ...
  <link rel="stylesheet" href="../assets/css/tw-10.css">
<style>
  /* Design system — same tokens as CCS.php / admin_page.php, gold-accented rail. */
  :root {
    --bg:#f5f6f8; --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(99,102,241,0.05), transparent 60%);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:32px; height:32px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--accent), #6366f1); display:flex; align-items:center; justify-content:center; }
  ...
  .scope-badge { display:inline-flex; align-items:center; gap:0.4rem; background:var(--gold-soft); color:#92400e; padding:0.3rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; margin-left:0.5rem; vertical-align:middle; }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--gold-soft); color:#92400e; padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .filter-menu a.sel { background:var(--gold-soft); color:#92400e; font-weight:600; }
  ...
  .pg.active { background:var(--gold); color:#fff; border-color:var(--gold); }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/ccs-logo.png" alt="CCS" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="CCS.php" class="rail-link">
      ...
      <a href="CCS_Assessment Form.php" class="rail-link active">
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>CCS Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> CCS only</span></h2>
            <p>Viewing submissions from College of Computer Studies faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
          <button onclick="generatePDF()" class="btn btn-gold">
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">CCS Records</p>
          ...
          <div class="stat a-amber">
            <div class="stat-body">
              ...
              <div class="stat-ico ico-amber"><i class="ri-user-line"></i></div>
            </div>
          </div>
          <div class="stat a-gold">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $ccs_eligible ?> CCS accepted users</p>
              </div>
              <div class="stat-ico ico-gold"><i class="ri-percent-line"></i></div>
            </div>
          </div>
        </div>

        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to CCS) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> CCS records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Computer Studies</div>
                      </td>
                      <td>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No CCS submissions found</h4>
                        <p>
...
<!-- HIDDEN EXPORT TABLE (CCS rows only) -->
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Computer Studies</p></div>
              ...
              <div class="profile-card" id="dean-recs-card">
                ...
                <p style="color:var(--muted);font-size:0.85rem;margin-bottom:1rem;">
                  What the AI recommended for this employee and where each stands. Accepted trainings are pooled with the rest of CCS under <strong>Training Demand Forwarded to You</strong> on the main dashboard once HR forwards them.
                </p>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF COMPUTER STUDIES";
  doc.setFontSize(13);
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[245,230,200], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });

  doc.save("CCS_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
