<?php
/* =============================================================================
   LSPU — CCS TRAINING DEMAND (dedicated page)
   -----------------------------------------------------------------------------
   2026-09-04 - split out of CCS.php's dashboard. The "Training Demand
   Forwarded to You" section used to be the LAST thing on the dashboard,
   below stat cards, two chart rows, and two evaluation lists - an
   actionable, time-sensitive item buried at the bottom of a long scroll.
   This gives it its own sidebar entry (mirrors HR's own dedicated
   Training Demand section in admin_page.php) so a dean can jump straight
   to it. CCS.php keeps a slim teaser card linking here instead of the
   full listing. Design system, PHP backend logic (forwardedDemandRows,
   canonicalTnaCollegeCode()-based cross-college breakdown), and the
   report-to-HR modal JS are all copied verbatim from CCS.php so the two
   pages stay pixel/behavior identical.
   ============================================================================= */
...
/* -----------------------------------------------------------------------------
   TRAINING DEMAND FORWARDED TO CCS — identical to CCS.php's own block.
   ----------------------------------------------------------------------------- */
...
<head>
  ...
  <title>CCS Training Demand · LSPU TNA</title>
  ...
  <!-- Design system copied 1:1 from CCS.php so both pages match exactly. -->
  <style>
    :root {
      --bg: #f5f6f8;
      ...
      --accent: #4f46e5; --accent-600: #4f46e5; --accent-700: #4338ca; --accent-soft: #eef2ff; --accent-ink: #3730a3;
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="CCS_Training_Demand.php" class="rail-link active">
    ...
    <header class="topbar">
      ...
      <div class="topbar-right">
        <!-- 2026-09-06 fix - the notification bell only belongs on the
             Dashboard (matching the established pattern already used by
             Assessment Form / IDP Forms / Evaluation, none of which have
             one). Removed here for consistency - see CLAUDE.md. -->
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <p>Trainings HR has forwarded to CCS because your own faculty/staff requested them — report back once you've found a provider.</p>
          </div>
        ...
        <p style="font-size:0.8rem;color:var(--muted);margin:-0.6rem 0 1.1rem;">
          Already found something on your own, with no employee request behind it? <a href="#" onclick="openPostOpportunityModal();return false;" style="color:var(--accent);font-weight:600;">Post it as an opportunity</a> instead of a group chat - your <?= htmlspecialchars('CCS') ?> staff will see it and can sign up right in the system.
        </p>
        </div>
          ...
          <div class="stat a-gold">
            <div class="stat-body">
              <div>
                ...
                <h3 class="stat-value num" style="color:#92400e;"><?= count($forwardedDemandRows) ?></h3>
              </div>
...
/* Training Demand — report a found paid training back to HR. Copied
   verbatim from CCS.php so behavior is identical on both pages. */
