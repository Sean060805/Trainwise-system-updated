<?php
/* ===================================================================
   CCS IDP SUBMISSIONS — Department view for College of Computer Studies
   =================================================================== */
...
function processSignatureUpload($file, $role, $formId)
{
    ...
    $uploadDir = '../uploads/signatures/ccs/';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_admin_signatures') {
    ...
    try {
        ...
        // 2026-09-10 fix - empty-string dates (any signer whose date field
        // wasn't filled, e.g. the dean uploads only the supervisor signature)
        // must bind as SQL NULL, not '', or the DATE columns raise a
        // truncation error under mysqli's exception mode - the same class of
        // failure the idp_certification.status enum was just widened to avoid.
        $employeeDate   = $certification['employee_date']   !== '' ? $certification['employee_date']   : null;
        $supervisorDate = $certification['supervisor_date'] !== '' ? $certification['supervisor_date'] : null;
        $directorDate   = $certification['director_date']   !== '' ? $certification['director_date']   : null;
...
/*
|--------------------------------------------------------------------------
| Main employee list - FILTERED TO CCS DEPARTMENT
|--------------------------------------------------------------------------
*/
$query = "
    SELECT 
    ...
    WHERE f.status = 'submitted' AND u.department = 'CCS'
...
$statsResult = $con->query("
    ...
    FROM users 
    WHERE department = 'CCS' AND role = 'user' 
    AND teaching_status IS NOT NULL AND teaching_status != ''
");
if ($statsResult) {
    $ccsStats['total_employees'] = $statsResult->fetch_assoc()['count'];
}
$ccsStats['with_idp'] = count($employees);
...
$hrResult = $con->query("
    ...
    FROM idp_forms f
    JOIN users u ON f.user_id = u.id
    WHERE f.status = 'submitted_to_hr' AND u.department = 'CCS'
");
if ($hrResult) {
    $ccsStats['submitted_to_hr'] = $hrResult->fetch_assoc()['count'];
}
...
<head>
  ...
  <title>CCS IDP Submissions · LSPU TNA</title>
  ...
  <!-- TAILWIND -->
  <link rel="stylesheet" href="../assets/css/tw-27.css">
<style>
  /* ==========================================================
     DESIGN SYSTEM — CCS Gold Accent Version
     ========================================================== */

  :root {
    --bg:#f5f6f8;
    --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(79,70,229,0.05), transparent 60%);
    ...
    --gold:#4f46e5; --gold-light:#818cf8; --gold-soft:#eef2ff;
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-link.active { background:linear-gradient(100deg, rgba(79,70,229,0.26), rgba(79,70,229,0.08)); color:#fff; border-color:rgba(129,140,248,0.35); }
  ...
  .ico-gold    { background:var(--gold-soft); color:#3730a3; }
  ...
  .btn-gold { background:var(--gold); color:#fff; box-shadow:0 8px 18px -10px rgba(79,70,229,0.6); }
  ...
  .view-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--gold); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.85rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(79,70,229,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
  .view-btn:hover { background:#B8922A; }
  ...
  .department-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--gold-soft); color:#3730a3; padding:0.32rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .emp-avatar { width:38px; height:38px; border-radius:10px; background:var(--gold-soft); color:#3730a3; display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .search input:focus { outline:none; border-color:var(--gold); box-shadow:0 0 0 4px rgba(79,70,229,0.12); }
  ...
  .search-go:hover { background:#B8922A; }
  ...
  .status-select:focus { outline:none; border-color:var(--gold); box-shadow:0 0 0 4px rgba(79,70,229,0.12); }
  ...
  .modal-table tr:hover td { background:var(--gold-soft); }
  .modal-table tr:last-child td { border-bottom:none; }
  .checkbox-custom { width:18px; height:18px; accent-color:var(--gold); }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="CCS_Individual_Development_Plan_Form.php" class="rail-link active">
  ...
  <div class="app-main">
    <header class="topbar">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip" aria-label="Account">
          ...
          <span class="avatar-meta"><span class="nm"><?= htmlspecialchars($adminName) ?></span> <span class="rl">CCS Admin</span></span>
        </div>
      </div>
    </header>
    ...
    <div class="content-scroll">
      <div class="content">
        <div class="page-head">
          <div>
            <p class="eyebrow">Individual Development Plans · CCS</p>
            <h2>CCS IDP Submissions</h2>
            <p>View and manage IDP forms submitted by College of Computer Studies faculty and staff.</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">Total CCS Faculty</p>
                <p class="stat-value num"><?php echo $ccsStats['total_employees']; ?></p>
          ...
          <div class="stat a-gold">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $ccsStats['with_idp']; ?></p>
                <p class="stat-sub"><?php echo $ccsStats['total_employees'] > 0 ? round(($ccsStats['with_idp'] / $ccsStats['total_employees']) * 100, 1) : 0; ?>% of faculty</p>
              </div>
              <div class="stat-ico ico-gold"><i class="ri-file-list-3-line"></i></div>
            </div>
          </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $ccsStats['submitted_to_hr']; ?></p>
        ...
        <div class="card" style="margin-bottom:1.4rem; overflow:visible; position:relative; z-index:20;">
          <div class="card-body" style="padding:1.05rem 1.15rem;">
            <div class="filterbar">
              <div style="display:inline-flex;align-items:center;gap:0.55rem;height:42px;padding:0 0.9rem;border:1px solid var(--gold);border-radius:var(--radius-sm);background:var(--gold-soft);">
                <i class="ri-building-line" style="color:#3730a3;"></i>
                <span style="font-weight:600;color:#3730a3;font-size:0.85rem;">CCS</span>
                <span style="font-size:0.7rem;color:#3730a3;opacity:0.7;">College of Computer Studies</span>
              </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p style="font-size:0.82rem; color:var(--muted);">
            ...
            of <span class="num"><?= count($employees) ?></span> CCS employees with submitted IDPs
          </p>
        </div>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No IDP submissions found for CCS</h4>
                        <p>CCS faculty and staff will appear here once they submit their IDP forms.</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($employees as $employee): ?>
                    ...
                    <tr class="employee-item" data-department="<?= htmlspecialchars($employee['department']) ?>">
                      ...
                      <td><span class="num" style="font-weight:700; color:var(--gold); font-size:1.05rem;"><?= (int) $employee['total_idps'] ?></span></td>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="signature-upload-card">
                    ...
                    <input
                      ...
                      class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-gold file:text-white hover:file:bg-[#B8922A] cursor-pointer signature-file-input"
  ...
  function showIDPModal(userId, userName, expandFormId = null) {
    ...
    modalContent.innerHTML = `
      <div class="flex justify-center items-center" style="padding:3rem 0;">
        <div style="width:36px;height:36px;border:3px solid var(--gold-soft);border-top-color:var(--gold);border-radius:50%;animation:spin 0.8s linear infinite;"></div>
      </div>
