<?php
/* ===================================================================
   CIT IDP SUBMISSIONS — Department view for College of Industrial Technology
   =================================================================== */
...
$forwardedDemandCount = getForwardedDemandCountForCollege($con, 'CIT');
...
function processSignatureUpload($file, $role, $formId)
{
    ...
    $uploadDir = '../uploads/signatures/cit/';
...
/*
|--------------------------------------------------------------------------
| Main employee list - FILTERED TO CIT DEPARTMENT
|--------------------------------------------------------------------------
*/
$query = "
    SELECT 
    ...
    WHERE f.status = 'submitted' AND u.department = 'CIT'
...
$statsResult = $con->query("
    ...
    FROM users 
    WHERE department = 'CIT' AND role = 'user' 
    AND teaching_status IS NOT NULL AND teaching_status != ''
");
if ($statsResult) {
    $citStats['total_employees'] = $statsResult->fetch_assoc()['count'];
}
$citStats['with_idp'] = count($employees);
...
$hrResult = $con->query("
    ...
    FROM idp_forms f
    JOIN users u ON f.user_id = u.id
    WHERE f.status = 'submitted_to_hr' AND u.department = 'CIT'
");
if ($hrResult) {
    $citStats['submitted_to_hr'] = $hrResult->fetch_assoc()['count'];
}
...
<head>
  ...
  <title>CIT IDP Submissions · LSPU TNA</title>
  ...
  <!-- TAILWIND -->
  <link rel="stylesheet" href="../assets/css/tw-19.css">
<style>
  /* ==========================================================
     DESIGN SYSTEM — CIT Red Accent Version
     ========================================================== */
  ...
  :root {
    ...
    --accent:#b91c1c; --accent-700:#991b1b; --accent-soft:#fef2f2; --accent-ink:#7f1d1d;
    --citred:#b91c1c; --citred-light:#dc2626; --citred-soft:#fef2f2;
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:38px; height:38px; border-radius:10px; flex-shrink:0; background:linear-gradient(135deg, var(--citred), var(--citred-light)); display:flex; align-items:center; justify-content:center; }
  ...
  .avatar { width:34px; height:34px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--citred), var(--citred-light)); color:#fff; font-weight:700; font-size:0.85rem; display:flex; align-items:center; justify-content:center; font-family:'Plus Jakarta Sans',sans-serif; }
  ...
  .date-chip i { color:var(--citred); font-size:1.05rem; }
  ...
  .stat::after { content:''; position:absolute; inset:0 0 auto 0; height:3px; background:var(--stat-accent, var(--citred)); }
  .stat.a-citred  { --stat-accent:linear-gradient(90deg, var(--citred-light), var(--citred)); }
  ...
  .ico-citred  { background:var(--citred-soft); color:#7f1d1d; }
  ...
  .btn-citred { background:var(--citred); color:#fff; box-shadow:0 8px 18px -10px rgba(185,28,28,0.6); }
  ...
  .view-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--citred); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.85rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(185,28,28,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
  .view-btn:hover { background:#991b1b; }
  ...
  .department-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--citred-soft); color:#7f1d1d; padding:0.32rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  table.data tbody tr:hover { background:var(--citred-soft); }
  table.data tbody tr:last-child td { border-bottom:none; }
  .cell-name { font-weight:600; color:var(--ink); }
  .emp-avatar { width:38px; height:38px; border-radius:10px; background:var(--citred-soft); color:#7f1d1d; display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .search input:focus { outline:none; border-color:var(--citred); box-shadow:0 0 0 4px rgba(185,28,28,0.12); }
  ...
  .search-go { position:absolute; right:6px; top:50%; transform:translateY(-50%); width:32px; height:32px; border:none; border-radius:8px; background:var(--citred); color:#fff; cursor:pointer; display:inline-flex; align-items:center; justify-content:center; transition:background 0.18s var(--ease); }
  .search-go:hover { background:#991b1b; }
  ...
  .filter.open .filter-btn { border-color:var(--citred); }
  ...
  .status-select:focus { outline:none; border-color:var(--citred); box-shadow:0 0 0 4px rgba(185,28,28,0.12); }
  ...
  .modal-table tr:hover td { background:var(--citred-soft); }
  .modal-table tr:last-child td { border-bottom:none; }
  .checkbox-custom { width:18px; height:18px; accent-color:var(--citred); }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="CIT.php" class="rail-link">
      ...
      <a href="CIT_Individual_Development_Plan_Form.php" class="rail-link active">
  ...
  <div class="app-main">
    <header class="topbar">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip" aria-label="Account">
          ...
          <span class="avatar-meta"><span class="nm"><?= htmlspecialchars($adminName) ?></span> <span class="rl">CIT Admin</span></span>
        </div>
      </div>
    </header>
    ...
    <div class="content-scroll">
      <div class="content">
        <div class="page-head">
          <div>
            <p class="eyebrow">Individual Development Plans · CIT</p>
            <h2>CIT IDP Submissions</h2>
            <p>View and manage IDP forms submitted by College of Industrial Technology faculty and staff.</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">Total CIT Faculty</p>
                <p class="stat-value num"><?php echo $citStats['total_employees']; ?></p>
                <p class="stat-sub">eligible for IDP</p>
              </div>
              <div class="stat-ico ico-indigo"><i class="ri-team-line"></i></div>
            </div>
          </div>
          <div class="stat a-citred">
            <div class="stat-body">
              <div>
                <p class="stat-label">With IDP Submitted</p>
                <p class="stat-value num"><?php echo $citStats['with_idp']; ?></p>
                <p class="stat-sub"><?php echo $citStats['total_employees'] > 0 ? round(($citStats['with_idp'] / $citStats['total_employees']) * 100, 1) : 0; ?>% of faculty</p>
              </div>
              <div class="stat-ico ico-citred"><i class="ri-file-list-3-line"></i></div>
            </div>
          </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $citStats['submitted_to_hr']; ?></p>
        ...
        <div class="card" style="margin-bottom:1.4rem; overflow:visible; position:relative; z-index:20;">
          <div class="card-body" style="padding:1.05rem 1.15rem;">
            <div class="filterbar">
              <div style="display:inline-flex;align-items:center;gap:0.55rem;height:42px;padding:0 0.9rem;border:1px solid var(--citred);border-radius:var(--radius-sm);background:var(--citred-soft);">
                <i class="ri-building-line" style="color:#7f1d1d;"></i>
                <span style="font-weight:600;color:#7f1d1d;font-size:0.85rem;">CIT</span>
                <span style="font-size:0.7rem;color:#7f1d1d;opacity:0.7;">College of Industrial Technology</span>
              </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p style="font-size:0.82rem; color:var(--muted);">
            ...
            of <span class="num"><?= count($employees) ?></span> CIT employees with submitted IDPs
          </p>
        </div>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No IDP submissions found for CIT</h4>
                        <p>CIT faculty and staff will appear here once they submit their IDP forms.</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($employees as $employee): ?>
                    ...
                    <tr class="employee-item" data-department="<?= htmlspecialchars($employee['department']) ?>">
                      ...
                      <td><span class="num" style="font-weight:700; color:var(--citred); font-size:1.05rem;"><?= (int) $employee['total_idps'] ?></span></td>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="signature-upload-card">
                    ...
                    <input
                      ...
                      class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-citred file:text-white hover:file:bg-[#991b1b] cursor-pointer signature-file-input"
  ...
  function showIDPModal(userId, userName, expandFormId = null) {
    ...
    modalContent.innerHTML = `
      <div class="flex justify-center items-center" style="padding:3rem 0;">
        <div style="width:36px;height:36px;border:3px solid var(--citred-soft);border-top-color:var(--citred);border-radius:50%;animation:spin 0.8s linear infinite;"></div>
      </div>
