<?php
/* ===================================================================
   COL IDP SUBMISSIONS — Department view for College of Law
   =================================================================== */
...
$forwardedDemandCount = getForwardedDemandCountForCollege($con, 'COL');
...
function processSignatureUpload($file, $role, $formId)
{
    ...
    $uploadDir = '../uploads/signatures/col/';
...
/*
|--------------------------------------------------------------------------
| Main employee list - FILTERED TO COL DEPARTMENT
|--------------------------------------------------------------------------
*/
$query = "
    SELECT 
    ...
    WHERE f.status = 'submitted' AND u.department = 'COL'
...
$statsResult = $con->query("
    ...
    FROM users 
    WHERE department = 'COL' AND role = 'user' 
    AND teaching_status IS NOT NULL AND teaching_status != ''
");
if ($statsResult) {
    $colStats['total_employees'] = $statsResult->fetch_assoc()['count'];
}
$colStats['with_idp'] = count($employees);
...
$hrResult = $con->query("
    ...
    FROM idp_forms f
    JOIN users u ON f.user_id = u.id
    WHERE f.status = 'submitted_to_hr' AND u.department = 'COL'
");
if ($hrResult) {
    $colStats['submitted_to_hr'] = $hrResult->fetch_assoc()['count'];
}
...
<head>
  ...
  <title>COL IDP Submissions · LSPU TNA</title>
  ...
  <!-- TAILWIND -->
  <link rel="stylesheet" href="../assets/css/tw-26.css">
<style>
  /* ==========================================================
     DESIGN SYSTEM — COL Violet/Gold Accent Version
     ========================================================== */
  ...
  :root {
    ...
    --accent:#4c1d95; --accent-700:#3b0f7a; --accent-soft:#f5f3ff; --accent-ink:#2e1065;
    --colviolet:#4c1d95; --colviolet-light:#6d28d9; --colviolet-soft:#f5f3ff;
    --colgold:#daa520; --colgold-light:#e8c96e; --colgold-soft:#fefce8;
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:38px; height:38px; border-radius:10px; flex-shrink:0; background:linear-gradient(135deg, var(--colviolet), var(--colviolet-light)); display:flex; align-items:center; justify-content:center; }
  ...
  .avatar { width:34px; height:34px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--colviolet), var(--colviolet-light)); color:#fff; font-weight:700; font-size:0.85rem; display:flex; align-items:center; justify-content:center; font-family:'Plus Jakarta Sans',sans-serif; }
  ...
  .date-chip i { color:var(--colviolet); font-size:1.05rem; }
  ...
  .stat::after { content:''; position:absolute; inset:0 0 auto 0; height:3px; background:var(--stat-accent, var(--colviolet)); }
  ...
  .btn-colviolet { background:var(--colviolet); color:#fff; box-shadow:0 8px 18px -10px rgba(76,29,149,0.6); }
  ...
  .view-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--colviolet); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.85rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(76,29,149,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
  .view-btn:hover { background:#3b0f7a; }
  ...
  .department-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--colviolet-soft); color:var(--colviolet); padding:0.32rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  table.data tbody tr:hover { background:var(--colviolet-soft); }
  table.data tbody tr:last-child td { border-bottom:none; }
  .cell-name { font-weight:600; color:var(--ink); }
  .emp-avatar { width:38px; height:38px; border-radius:10px; background:var(--colviolet-soft); color:var(--colviolet); display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .search input:focus { outline:none; border-color:var(--colviolet); box-shadow:0 0 0 4px rgba(76,29,149,0.12); }
  ...
  .search-go { position:absolute; right:6px; top:50%; transform:translateY(-50%); width:32px; height:32px; border:none; border-radius:8px; background:var(--colviolet); color:#fff; cursor:pointer; display:inline-flex; align-items:center; justify-content:center; transition:background 0.18s var(--ease); }
  .search-go:hover { background:#3b0f7a; }
  ...
  .filter.open .filter-btn { border-color:var(--colviolet); }
  ...
  .status-select:focus { outline:none; border-color:var(--colviolet); box-shadow:0 0 0 4px rgba(76,29,149,0.12); }
  ...
  .modal-table tr:hover td { background:var(--colviolet-soft); }
  .modal-table tr:last-child td { border-bottom:none; }
  .checkbox-custom { width:18px; height:18px; accent-color:var(--colviolet); }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="COL.php" class="rail-link">
      ...
      <a href="COL_Individual_Development_Plan_Form.php" class="rail-link active">
  ...
  <div class="app-main">
    <header class="topbar">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip" aria-label="Account">
          ...
          <span class="avatar-meta"><span class="nm"><?= htmlspecialchars($adminName) ?></span> <span class="rl">COL Admin</span></span>
        </div>
      </div>
    </header>
    ...
    <div class="content-scroll">
      <div class="content">
        <div class="page-head">
          <div>
            <p class="eyebrow">Individual Development Plans · COL</p>
            <h2>COL IDP Submissions</h2>
            <p>View and manage IDP forms submitted by College of Law faculty and staff.</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">Total COL Faculty</p>
                <p class="stat-value num"><?php echo $colStats['total_employees']; ?></p>
                <p class="stat-sub">eligible for IDP</p>
              </div>
              <div class="stat-ico ico-indigo"><i class="ri-team-line"></i></div>
            </div>
          </div>
          <div class="stat a-colviolet">
            <div class="stat-body">
              <div>
                <p class="stat-label">With IDP Submitted</p>
                <p class="stat-value num"><?php echo $colStats['with_idp']; ?></p>
                <p class="stat-sub"><?php echo $colStats['total_employees'] > 0 ? round(($colStats['with_idp'] / $colStats['total_employees']) * 100, 1) : 0; ?>% of faculty</p>
              </div>
              <div class="stat-ico ico-colviolet"><i class="ri-file-list-3-line"></i></div>
            </div>
          </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $colStats['submitted_to_hr']; ?></p>
        ...
        <div class="card" style="margin-bottom:1.4rem; overflow:visible; position:relative; z-index:20;">
          <div class="card-body" style="padding:1.05rem 1.15rem;">
            <div class="filterbar">
              <div style="display:inline-flex;align-items:center;gap:0.55rem;height:42px;padding:0 0.9rem;border:1px solid var(--colviolet);border-radius:var(--radius-sm);background:var(--colviolet-soft);">
                <i class="ri-building-line" style="color:var(--colviolet);"></i>
                <span style="font-weight:600;color:var(--colviolet);font-size:0.85rem;">COL</span>
                <span style="font-size:0.7rem;color:var(--colviolet);opacity:0.7;">College of Law</span>
              </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p style="font-size:0.82rem; color:var(--muted);">
            ...
            of <span class="num"><?= count($employees) ?></span> COL employees with submitted IDPs
          </p>
        </div>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No IDP submissions found for COL</h4>
                        <p>COL faculty and staff will appear here once they submit their IDP forms.</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($employees as $employee): ?>
                    ...
                    <tr class="employee-item" data-department="<?= htmlspecialchars($employee['department']) ?>">
                      ...
                      <td><span class="num" style="font-weight:700; color:var(--colviolet); font-size:1.05rem;"><?= (int) $employee['total_idps'] ?></span></td>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="signature-upload-card">
                    ...
                    <input
                      ...
                      class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-colviolet file:text-white hover:file:bg-[#3b0f7a] cursor-pointer signature-file-input"
  ...
  function showIDPModal(userId, userName, expandFormId = null) {
    ...
    modalContent.innerHTML = `
      <div class="flex justify-center items-center" style="padding:3rem 0;">
        <div style="width:36px;height:36px;border:3px solid var(--colviolet-soft);border-top-color:var(--colviolet);border-radius:50%;animation:spin 0.8s linear infinite;"></div>
      </div>
