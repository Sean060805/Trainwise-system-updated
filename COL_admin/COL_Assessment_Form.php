<?php
/* ===================================================================
   COL ASSESSMENT FORM SUBMISSIONS — College of Law only
   -------------------------------------------------------------------
   This is a COL-scoped copy of "CCS_Assessment Form.php". EVERY query below
   is constrained to department = 'COL' — a COL admin can only ever see
   or act on COL records. UI matches COL.php (dark rail sidebar, violet/gold
   accent, dual LSPU + COL logos).

   Assumed to live in the same folder as COL.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as COL.php — keeps this page inside the protected COL area.
...
// ----- Delete handler (POST) — only allowed for COL-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a COL user.
        $colDept = COL_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $colDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — COL only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . COL_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — COL only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . COL_DEPT . "'";
...
// ----- Count total rows (respects active filters, COL only) ---------
...
// ----- Main paginated query (sorted by surname, COL only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all COL rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted COL faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always COL).
$col_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . COL_DEPT . "' AND status = 'accepted'");
$col_eligible = ($col_eligible_result && $col_eligible_result->num_rows > 0) ? (int)$col_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $col_eligible > 0 ? round(($total_rows / $col_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>COL Assessment Forms · LSPU TNA</title>
  ...
  <link rel="stylesheet" href="../assets/css/tw-16.css">
<style>
  /* Design system — same tokens as COL.php / admin_page.php, violet/gold-accented rail. */
  :root {
    --bg:#f5f6f8;
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:32px; height:32px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--accent), #6d28d9); display:flex; align-items:center; justify-content:center; }
  ...
  .scope-badge { display:inline-flex; align-items:center; gap:0.4rem; background:var(--colviolet-soft); color:#2e1065; padding:0.3rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; margin-left:0.5rem; vertical-align:middle; }
  ...
  .stat.a-colviolet { --stat-accent:linear-gradient(90deg, var(--colviolet), #6d28d9); }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--colviolet-soft); color:#2e1065; padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .search input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(76,29,149,0.12); }
  ...
  .filter-menu a.sel { background:var(--colviolet-soft); color:#2e1065; font-weight:600; }
  ...
  .pg.active { background:var(--colviolet); color:#fff; border-color:var(--colviolet); }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/col-logo.png" alt="COL" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="COL_Assessment_Form.php" class="rail-link active">
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>COL Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> COL only</span></h2>
            <p>Viewing submissions from College of Law faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
          <button onclick="generatePDF()" class="btn btn-colviolet">
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">COL Records</p>
          ...
          <div class="stat a-colviolet">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $col_eligible ?> COL accepted users</p>
              </div>
              <div class="stat-ico ico-colviolet"><i class="ri-percent-line"></i></div>
            </div>
          </div>
        </div>

        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to COL) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> COL records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Law</div>
                      </td>
                      <td>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No COL submissions found</h4>
                        <p>
...
<!-- HIDDEN EXPORT TABLE (COL rows only) -->
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Law</p></div>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF LAW";
  doc.setFontSize(13);
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[245,243,255], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });

  doc.save("COL_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
