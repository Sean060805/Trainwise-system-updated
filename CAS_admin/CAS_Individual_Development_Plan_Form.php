<?php
/* ===================================================================
   CAS IDP SUBMISSIONS — Department view for College of Arts & Sciences
   =================================================================== */
...
$forwardedDemandCount = getForwardedDemandCountForCollege($con, 'CAS');
...
function processSignatureUpload($file, $role, $formId)
{
    ...
    $uploadDir = '../uploads/signatures/cas/';
...
/*
|--------------------------------------------------------------------------
| Main employee list - FILTERED TO CAS DEPARTMENT
|--------------------------------------------------------------------------
*/
$query = "
    SELECT 
    ...
    WHERE f.status = 'submitted' AND u.department = 'CAS'
...
$statsResult = $con->query("
    ...
    FROM users 
    WHERE department = 'CAS' AND role = 'user' 
    AND teaching_status IS NOT NULL AND teaching_status != ''
");
if ($statsResult) {
    $casStats['total_employees'] = $statsResult->fetch_assoc()['count'];
}
$casStats['with_idp'] = count($employees);
...
$hrResult = $con->query("
    ...
    FROM idp_forms f
    JOIN users u ON f.user_id = u.id
    WHERE f.status = 'submitted_to_hr' AND u.department = 'CAS'
");
if ($hrResult) {
    $casStats['submitted_to_hr'] = $hrResult->fetch_assoc()['count'];
}
...
<head>
  ...
  <title>CAS IDP Submissions · LSPU TNA</title>
  ...
  <!-- TAILWIND -->
  <link rel="stylesheet" href="../assets/css/tw-24.css">
<style>
  /* ==========================================================
     DESIGN SYSTEM — CAS Purple Accent Version
     ========================================================== */
  ...
  :root {
    ...
    --accent:#6d28d9; --accent-700:#5b21b6; --accent-soft:#ede9fe; --accent-ink:#5b21b6;
    ...
    --rail:#1a0a2e; --rail-2:#2d1b4e; --rail-line:rgba(148,163,184,0.14);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:38px; height:38px; border-radius:10px; flex-shrink:0; background:linear-gradient(135deg, var(--purple), var(--purple-light)); display:flex; align-items:center; justify-content:center; }
  ...
  .avatar { width:34px; height:34px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--purple), var(--purple-light)); color:#fff; font-weight:700; font-size:0.85rem; display:flex; align-items:center; justify-content:center; font-family:'Plus Jakarta Sans',sans-serif; }
  ...
  .date-chip i { color:var(--purple); font-size:1.05rem; }
  ...
  .stat::after { content:''; position:absolute; inset:0 0 auto 0; height:3px; background:var(--stat-accent, var(--purple)); }
  .stat.a-purple   { --stat-accent:linear-gradient(90deg, var(--purple-light), var(--purple)); }
  ...
  .ico-purple   { background:var(--purple-soft); color:#4c1d95; }
  ...
  .btn-purple { background:var(--purple); color:#fff; box-shadow:0 8px 18px -10px rgba(109,40,217,0.6); }
  ...
  .view-btn { display:inline-flex; align-items:center; gap:0.4rem; background:var(--purple); color:#fff; padding:0.5rem 0.95rem; border-radius:var(--radius-sm); font-size:0.85rem; font-weight:600; border:none; cursor:pointer; box-shadow:0 8px 18px -10px rgba(109,40,217,0.6); transition:background 0.18s var(--ease), transform 0.12s var(--ease); }
  .view-btn:hover { background:#4c1d95; }
  ...
  .department-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--purple-soft); color:#4c1d95; padding:0.32rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  table.data tbody tr:hover { background:var(--purple-soft); }
  table.data tbody tr:last-child td { border-bottom:none; }
  .cell-name { font-weight:600; color:var(--ink); }
  .emp-avatar { width:38px; height:38px; border-radius:10px; background:var(--purple-soft); color:#4c1d95; display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .search input:focus { outline:none; border-color:var(--purple); box-shadow:0 0 0 4px rgba(109,40,217,0.12); }
  ...
  .search-go { position:absolute; right:6px; top:50%; transform:translateY(-50%); width:32px; height:32px; border:none; border-radius:8px; background:var(--purple); color:#fff; cursor:pointer; display:inline-flex; align-items:center; justify-content:center; transition:background 0.18s var(--ease); }
  .search-go:hover { background:#4c1d95; }
  ...
  .filter.open .filter-btn { border-color:var(--purple); }
  ...
  .status-select:focus { outline:none; border-color:var(--purple); box-shadow:0 0 0 4px rgba(109,40,217,0.12); }
  ...
  .modal-table tr:hover td { background:var(--purple-soft); }
  .modal-table tr:last-child td { border-bottom:none; }
  .checkbox-custom { width:18px; height:18px; accent-color:var(--purple); }
...
<div class="app" id="app">
  ...
  <aside class="rail" id="rail">
    ...
    <nav class="rail-nav">
      ...
      <a href="CAS.php" class="rail-link">
      ...
      <a href="CAS_Individual_Development_Plan_Form.php" class="rail-link active">
  ...
  <div class="app-main">
    <header class="topbar">
      ...
      <div class="topbar-right">
        ...
        <div class="avatar-chip" aria-label="Account">
          ...
          <span class="avatar-meta"><span class="nm"><?= htmlspecialchars($adminName) ?></span> <span class="rl">CAS Admin</span></span>
        </div>
      </div>
    </header>
    ...
    <div class="content-scroll">
      <div class="content">
        <div class="page-head">
          <div>
            <p class="eyebrow">Individual Development Plans · CAS</p>
            <h2>CAS IDP Submissions</h2>
            <p>View and manage IDP forms submitted by College of Arts & Sciences faculty and staff.</p>
          </div>
        </div>
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">Total CAS Faculty</p>
                <p class="stat-value num"><?php echo $casStats['total_employees']; ?></p>
                <p class="stat-sub">eligible for IDP</p>
              </div>
              <div class="stat-ico ico-indigo"><i class="ri-team-line"></i></div>
            </div>
          </div>
          <div class="stat a-purple">
            <div class="stat-body">
              <div>
                <p class="stat-label">With IDP Submitted</p>
                <p class="stat-value num"><?php echo $casStats['with_idp']; ?></p>
                <p class="stat-sub"><?php echo $casStats['total_employees'] > 0 ? round(($casStats['with_idp'] / $casStats['total_employees']) * 100, 1) : 0; ?>% of faculty</p>
              </div>
              <div class="stat-ico ico-purple"><i class="ri-file-list-3-line"></i></div>
            </div>
          </div>
          ...
          <div class="stat a-sky">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-value num"><?php echo $casStats['submitted_to_hr']; ?></p>
        ...
        <div class="card" style="margin-bottom:1.4rem; overflow:visible; position:relative; z-index:20;">
          <div class="card-body" style="padding:1.05rem 1.15rem;">
            <div class="filterbar">
              <div style="display:inline-flex;align-items:center;gap:0.55rem;height:42px;padding:0 0.9rem;border:1px solid var(--purple);border-radius:var(--radius-sm);background:var(--purple-soft);">
                <i class="ri-building-line" style="color:#4c1d95;"></i>
                <span style="font-weight:600;color:#4c1d95;font-size:0.85rem;">CAS</span>
                <span style="font-size:0.7rem;color:#4c1d95;opacity:0.7;">College of Arts & Sciences</span>
              </div>
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p style="font-size:0.82rem; color:var(--muted);">
            ...
            of <span class="num"><?= count($employees) ?></span> CAS employees with submitted IDPs
          </p>
        </div>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No IDP submissions found for CAS</h4>
                        <p>CAS faculty and staff will appear here once they submit their IDP forms.</p>
                      </div>
                    </td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($employees as $employee): ?>
                    ...
                    <tr class="employee-item" data-department="<?= htmlspecialchars($employee['department']) ?>">
                      ...
                      <td><span class="num" style="font-weight:700; color:var(--purple); font-size:1.05rem;"><?= (int) $employee['total_idps'] ?></span></td>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="signature-upload-card">
                    ...
                    <input
                      ...
                      class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-purple file:text-white hover:file:bg-[#4c1d95] cursor-pointer signature-file-input"
  ...
  function showIDPModal(userId, userName, expandFormId = null) {
    ...
    modalContent.innerHTML = `
      <div class="flex justify-center items-center" style="padding:3rem 0;">
        <div style="width:36px;height:36px;border:3px solid var(--purple-soft);border-top-color:var(--purple);border-radius:50%;animation:spin 0.8s linear infinite;"></div>
      </div>
