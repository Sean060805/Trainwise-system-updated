<?php
/* ===================================================================
   CAS — ASSESSMENT FORM SUBMISSIONS (College of Arts & Sciences only)
   -------------------------------------------------------------------
   This is a CAS-scoped copy of "Assessment Form.php". EVERY query below
   is constrained to department = 'CAS' — a CAS admin can only ever see
   or act on CAS records. UI matches CAS.php (dark rail sidebar, purple
   accent, dual LSPU + CAS logos).

   Assumed to live in the same folder as CAS.php (hence '../config.php',
   '../images/...' below). Move the require_once path if that's wrong.
   =================================================================== */

session_start();

// Same auth gate as CAS.php — keeps this page inside the protected CAS area.
...
// ----- Delete handler (POST) — only allowed for CAS-owned records ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $delete_id = intval($_POST['delete_id']);
    if ($delete_id > 0) {
        // Ownership check: the assessment must belong to a CAS user.
        $casDept = CAS_DEPT;
        ...
        $own_check->bind_param("is", $delete_id, $casDept);
        $own_check->execute();
...
// ----- Distinct years / months / statuses — CAS only ----------------
$years_result = $con->query("
    SELECT DISTINCT YEAR(a.submission_date) AS year
    FROM assessments a
    JOIN users u ON a.user_id = u.id
    WHERE u.department = '" . CAS_DEPT . "' AND a.submission_date IS NOT NULL
    ORDER BY year ASC
");
...
// ----- Detail lookup for the View modal (server-side fallback) — CAS only
$view_data = null;
if ($view_id > 0) {
    $view_sql = "SELECT
                    u.*,
                ...
                WHERE a.id = $view_id AND u.department = '" . CAS_DEPT . "'";
...
// ----- Count total rows (respects active filters, CAS only) ---------
...
// ----- Main paginated query (sorted by surname, CAS only) ------------
$sql = "SELECT
            u.id as user_id,
            u.name,
            u.department,
...
// ----- Export query (all CAS rows, no pagination, same sort) ---------
$export_sql = "SELECT
                  u.id as user_id,
                  u.name,
                  u.department,
...
// accepted CAS faculty/staff, so the 4th stat card carries real signal
// now that "Departments" is meaningless (it's always CAS).
$cas_eligible_result = $con->query("SELECT COUNT(*) AS c FROM users WHERE department = '" . CAS_DEPT . "' AND status = 'accepted'");
$cas_eligible = ($cas_eligible_result && $cas_eligible_result->num_rows > 0) ? (int)$cas_eligible_result->fetch_assoc()['c'] : 0;
$response_rate = $cas_eligible > 0 ? round(($total_rows / $cas_eligible) * 100, 1) : 0;
...
<head>
  ...
  <title>CAS Assessment Forms · LSPU TNA</title>
...
<style>
  /* Design system — same tokens as CAS.php / admin_page.php, purple-accented rail. */
  :root {
    --bg:#f5f6f8; --bg-grad:radial-gradient(1100px 600px at 100% -8%, rgba(109,40,217,0.05), transparent 60%);
  ...
  *,*::before,*::after { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  .rail-logo-fallback { width:32px; height:32px; border-radius:9px; flex-shrink:0; background:linear-gradient(135deg, var(--accent), #8b5cf6); display:flex; align-items:center; justify-content:center; }
  ...
  .scope-badge { display:inline-flex; align-items:center; gap:0.4rem; background:var(--purple-soft); color:#4c1d95; padding:0.3rem 0.7rem; border-radius:999px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; margin-left:0.5rem; vertical-align:middle; }
  ...
  .dept-badge { display:inline-flex; align-items:center; gap:0.3rem; background:var(--purple-soft); color:#4c1d95; padding:0.28rem 0.6rem; border-radius:8px; font-size:0.72rem; font-weight:700; letter-spacing:0.02em; }
  ...
  .search input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 4px rgba(109,40,217,0.12); }
  ...
  .filter-menu a.sel { background:var(--purple-soft); color:#4c1d95; font-weight:600; }
  ...
  .pg.active { background:var(--purple); color:#fff; border-color:var(--purple); }
  ...
  <aside class="rail" id="rail">
    <div class="rail-brand">
      <div style="display:flex;align-items:center;gap:0.35rem;flex-shrink:0;">
        ...
        <img src="../images/cas-logo.png" alt="CAS" class="rail-logo"
             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        <div class="rail-logo-fallback" style="display:none;width:32px;height:32px;">
    ...
    <nav class="rail-nav">
      ...
      <a href="CAS_Assessment Form.php" class="rail-link active">
      ...
      <div class="content">
        ...
        <div class="page-head">
          <div>
            ...
            <h2>CAS Assessment Form Submissions <span class="scope-badge"><i class="ri-shield-check-line"></i> CAS only</span></h2>
            <p>Viewing submissions from College of Arts & Sciences faculty &amp; staff for <span class="num"><?= htmlspecialchars($period_label) ?></span>.</p>
          </div>
          <button onclick="generatePDF()" class="btn btn-purple">
        ...
        <div class="grid-stats">
          <div class="stat a-indigo">
            <div class="stat-body">
              <div>
                <p class="stat-label">CAS Records</p>
          ...
          <div class="stat a-purple">
            <div class="stat-body">
              <div>
                ...
                <p class="stat-sub"><?= $total_rows ?> of <?= $cas_eligible ?> CAS accepted users</p>
              </div>
              <div class="stat-ico ico-purple"><i class="ri-percent-line"></i></div>
            </div>
          </div>
        </div>

        <!-- FILTER BAR (Year / Month / Status / Search — no Department filter, it's fixed to CAS) -->
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.8rem;">
          <p class="pager-info">
            ...
            · <span class="num"><?= $total_rows ?></span> CAS records
          </p>
        </div>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="cell-sub">College of Arts & Sciences</div>
                      </td>
                      <td>
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="empty">
                        ...
                        <h4>No CAS submissions found</h4>
                        <p>
...
<!-- HIDDEN EXPORT TABLE (CAS rows only) -->
            ...
            modalContent.innerHTML = `
              <div class="profile-card" style="margin-bottom:1.25rem;">
                <div class="flex flex-col md:flex-row gap-6">
                  ...
                  <div class="md:w-1/3">
                    <div class="space-y-4">
                      <div><h5 class="text-sm font-medium" style="color:var(--muted);">Department</h5><p style="color:var(--ink-2);">College of Arts & Sciences</p></div>
...
function generatePDF() {
  ...
  const title = "SUMMARY OF TRAINING NEEDS ASSESSMENT FORMS — COLLEGE OF ARTS & SCIENCES";
  doc.setFontSize(13);
  ...
  doc.autoTable({
    ...
    headStyles: { fillColor:[237,233,254], textColor:[0,0,0], fontStyle:'bold', lineWidth:0.2, lineColor:[0,0,0] },
    theme: 'grid'
  });

  doc.save("CAS_Summary_of_Training_Needs_Assessment_Forms.pdf");
}
</script>
