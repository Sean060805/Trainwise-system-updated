...
// again here unconditionally threw a "session already active" notice that
// leaked into this form's rendered output whenever loaded via iframe from
// an already-session-started parent page (e.g. any *_eval.php).
...
// 2026-09-03 fix - was $_GET only. The AJAX save posts to this same
// file via a bare relative URL (`fetch('training_program_impact_...',
// {method:'POST', body: formData})`, no query string), so $_GET is
// completely empty on that request even though the form's own hidden
// `evaluation_id` field (line ~342) correctly carries the real ID in
// $_POST. Reading $_GET only meant $evaluationId was always '' during
// a save, so the "update existing" branch below (`!empty($evaluationId)
// && $existing_evaluation`) never matched and every "Continue" on a
// draft silently inserted a brand-new duplicate evaluations row instead
// of updating the one being edited. Found via a live Playwright
// verification pass, confirmed against real DB rows (Continue on
// existing draft id=5 created a new id=6, left id=5 untouched).
$evaluationId = $_POST['evaluation_id'] ?? ($_GET['evaluation_id'] ?? '');
...
<head>
    ...
    <link rel="stylesheet" href="assets/css/tw-49.css">
        ...
        <form action="training_program_impact_assessment_form.php" method="POST" id="assessment-form">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div>
                    ...
                    <?php
                      // 2026-09-03 fix - this was hardcoded to "CCS"
                      // regardless of which college's dean opened the
                      // form (this file is shared across every college
                      // via ../training_program_impact_assessment_form.php,
                      // not CCS-specific) - every printed/saved evaluation
                      // for every other college recorded the wrong
                      // department. $user_data (the employee being
                      // evaluated) is the right source; falls back to the
                      // raw GET param if that lookup didn't run.
                      $displayDepartment = $user_data['department'] ?? ($_GET['department'] ?? '');
                    ?>
                    <input type="text" name="department" class="form-field w-full px-4 py-2 rounded" value="<?= htmlspecialchars($displayDepartment) ?>" readonly>
                </div>
                <div>
            ...
            <div class="mt-8 flex justify-end space-x-4">
                ...
                <button type="submit" name="action" value="print" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-button hover:bg-gray-100 transition-colors whitespace-nowrap">Print</button>
                <button type="submit" id="submit-form" name="action" value="submit" class="px-6 py-2 bg-primary text-white rounded-button hover:bg-primary-dark transition-colors whitespace-nowrap">Save as Draft</button>
            </div>
        </form>
    </div> 
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                // Print was a real, silent bug (2026-09-01): this branch
                // used to not exist at all, so clicking Print fell through
                // to the browser's native full-page form submission - the
                // iframe (or tab) navigated straight to the server's raw
                // JSON acknowledgement text ({"success":true,"message":
                // "Form ready for printing!"}) instead of ever printing
                // the actual filled-in form. The print action doesn't
                // persist anything server-side anyway (see the PHP handler
                // above), so there's nothing worth a round-trip for -
                // just print what's already on screen.
                ...
                if (e.submitter && e.submitter.value === 'submit') {
                    ...
                    // The actual root cause of "An error occurred while
                    // saving" (2026-09-01, confirmed via the diagnostic
                    // added above): FormData(form) alone does NOT include
                    // the clicked submit button's name/value pair unless
                    // you pass the button explicitly - it's only included
                    // automatically for a real native form submission.
                    // Without this, `action` was silently missing from the
                    // POST body entirely, so the server's
                    // `$_POST['action'] === 'submit'` check never matched,
                    // execution fell through past every exit() in the POST
                    // handler, and the full HTML page rendered as the
                    ...
                    // 2026-09-01: a real "An error occurred while saving"
                    // was reported from the dashboard-launched (iframe)
                    // path specifically, but a direct reproduction of this
                    // exact POST (real session, real ratings, real
                    // signature data) against this same endpoint returned
                    // a clean 200 with valid JSON - no PHP warning, no
                    // malformed output. Couldn't confirm the exact trigger,
                    // so rather than guess at a fix, this at least makes
                    // the failure diagnosable if it happens again: reads
                    // the raw response text first (so a non-JSON response -
                    // an HTML error page, a stray PHP notice - shows up
                    // directly in the alert instead of being swallowed
                    // into a generic message), and reports the real HTTP
                    // status/error instead of a fixed string.
                    .then(response => {
                        return response.text().then(text => {
                            if (!response.ok) {
                                throw new Error(`Server returned HTTP ${response.status}: ${text.slice(0, 300)}`);
                            }
                            try {
                                return JSON.parse(text);
                            } catch (e) {
                                throw new Error(`Server response wasn't valid JSON: ${text.slice(0, 300)}`);
                            }
                        });
                    })
                    .then(data => {
